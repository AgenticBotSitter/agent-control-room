...
interface CronExpression { minute: CronField; hour: CronField; dayOfMonth: CronField; month: CronField; dayOfWeek: CronField; }
...
function parseField(value: string, minimum: number, maximum: number, normalize: (value: number) => number = (number) => number): CronField | undefined {
  if (!value) return undefined;
  const values = new Set<number>();
  for (const segment of value.split(",")) {
    const [range, stepPart] = segment.split("/");
    if (segment.split("/").length > 2 || !range || (stepPart !== undefined && !stepPart)) return undefined;
    const step = stepPart === undefined ? 1 : parseNumber(stepPart);
    if (!step || step <= 0 || step > maximum - minimum + 1) return undefined;
    let start: number;
    let end: number;
    if (range === "*") { start = minimum; end = maximum; }
      ...
      start = parseNumber(bounds[0]) ?? Number.NaN;
      end = bounds.length === 2 ? parseNumber(bounds[1]) ?? Number.NaN : start;
      if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || start < minimum || end > maximum || end < start) return undefined;
    }
    for (let current = start; current <= end; current += step) values.add(normalize(current));
  }
  return { values, wildcard: value === "*" };
}

function parseCron(expression: string): CronExpression | undefined {
  const parts = expression.trim().split(/\s+/);
  if (parts.length !== 5) return undefined;
  const [minute, hour, dayOfMonth, month, dayOfWeek] = parts;
  const dow = parseField(dayOfWeek, 0, 7, (value) => value === 7 ? 0 : value);
  const result = {
  ...
  return result.minute && result.hour && result.dayOfMonth && result.month && result.dayOfWeek ? result as CronExpression : undefined;
}
...
function cronMatches(cron: CronExpression, local: LocalParts): boolean {
  ...
  const dowMatch = cron.dayOfWeek.values.has(local.dayOfWeek);
  const dayMatch = cron.dayOfMonth.wildcard && cron.dayOfWeek.wildcard ? true
    : cron.dayOfMonth.wildcard ? dowMatch
      : cron.dayOfWeek.wildcard ? domMatch
        : domMatch || dowMatch;
  return dayMatch;
}
...
function calculateCron(definition: ScheduleDefinitionV1, start: number, end: number): ScheduleOccurrenceV1[] | undefined {
  const cron = parseCron(definition.expression);
  ...
  for (let current = firstMinute; current < end; current += 60_000) {
    ...
    if (cronMatches(cron, local) && !seenLocalTimes.has(local.key)) {
