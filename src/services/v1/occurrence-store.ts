...
function payload(input: ScheduleOccurrenceProposalV1) {
  return { scheduleId: input.scheduleId, occurrenceKey: input.occurrenceKey,
    targetType: input.targetType, targetId: input.targetId, scheduledFor: input.scheduledFor,
...
export class ScheduleOccurrenceStore {
  ...
  async materialize(input: ScheduleOccurrenceProposalV1): Promise<{ occurrence: StoredScheduleOccurrenceV1; replayed: boolean }> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (inserted.rows[0]) {
        ...
        await tx.query(`INSERT INTO control_outbox (id,tenant_id,topic,aggregate_type,aggregate_id,idempotency_key,status,available_at,payload) VALUES ($1,$2,'schedule.occurrence.created','schedule_occurrence',$3,$4,'pending',$5,$6::jsonb)`, [outboxId(input),input.tenantId,input.occurrenceKey,input.occurrenceKey,input.createdAt,json(payload(input))]);
  ...
  async acknowledgeDelivery(input: { tenantId: string; scheduleId: string; occurrenceKey: string; deliveredAt: string }): Promise<{ occurrence: StoredScheduleOccurrenceV1; replayed: boolean }> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      const delivered = await tx.query<{ id: string }>(`SELECT id FROM control_outbox WHERE tenant_id=$1 AND id IN ($2,$3) AND status='delivered'
        AND topic='schedule.occurrence.created' AND aggregate_type='schedule_occurrence'
        AND aggregate_id=$4 AND idempotency_key=$4 AND payload=$5::jsonb`,
      [input.tenantId,outboxId(occurrence),legacyOutboxId(occurrence),occurrence.occurrenceKey,json(payload(occurrence))]);
  ...
  async reconcileDelivered(input: { tenantId: string; deliveredAt: string }): Promise<number> {
    ...
    const result = await this.db.query<{ occurrence_key: string }>(`UPDATE control_schedule_occurrences s SET state='dispatched',dispatched_at=$2 WHERE s.tenant_id=$1 AND s.state='pending' AND EXISTS (
      SELECT 1 FROM control_outbox o WHERE o.tenant_id=s.tenant_id AND o.id IN (
        'outbox:schedule:v2:' || encode(convert_to(char_length(s.tenant_id)::text || ':' || s.tenant_id || char_length(s.schedule_id)::text || ':' || s.schedule_id || char_length(s.occurrence_key)::text || ':' || s.occurrence_key,'UTF8'),'hex'),
        'outbox:schedule:' || s.tenant_id || ':' || s.schedule_id || ':' || s.occurrence_key)
      AND o.status='delivered' AND o.topic='schedule.occurrence.created' AND o.aggregate_type='schedule_occurrence'
      AND o.aggregate_id=s.occurrence_key AND o.idempotency_key=s.occurrence_key
      AND o.payload=jsonb_build_object('scheduleId',s.schedule_id,'occurrenceKey',s.occurrence_key,
        'targetType',s.target_type,'targetId',s.target_id,'definitionDigest',s.definition_digest,
        'scheduledFor',to_char(s.scheduled_for AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'))
      ) RETURNING s.occurrence_key`, [input.tenantId,input.deliveredAt]);
...
function legacyOutboxId(input: ScheduleOccurrenceProposalV1): string { return `outbox:schedule:${input.tenantId}:${input.scheduleId}:${input.occurrenceKey}`; }
function outboxId(input: ScheduleOccurrenceProposalV1): string {
  const tuple = [input.tenantId,input.scheduleId,input.occurrenceKey].map(value => `${value.length}:${value}`).join("");
  return `outbox:schedule:v2:${Buffer.from(tuple, "utf8").toString("hex")}`;
}
