...
export interface ServiceIncidentV1 { id: string; tenantId: string; correlationKey: string; generation: number; serviceId: string; severity: "warning" | "critical"; safeReasonCode: string; state: "open" | "resolved"; openedAt: string; lastObservedAt: string; resolvedAt?: string; }
export type ServiceIncidentInputV1 = { tenantId: string; serviceId: string; correlationKey: string; observedAt: string } & ({ action: "open_or_update"; severity: "warning" | "critical"; safeReasonCode: string } | { action: "resolve" });
export class ServiceIncidentError extends Error { constructor(readonly safeCode: "invalid_incident") { super(safeCode); } }

function valid(input: ServiceIncidentInputV1): boolean { return [input.tenantId,input.serviceId,input.correlationKey].every((value) => safeId.test(value)) && instant(input.observedAt) && (input.action === "resolve" || (safeId.test(input.safeReasonCode) && ["warning","critical"].includes(input.severity))); }
  ...
  async apply(input: ServiceIncidentInputV1): Promise<{ incident?: ServiceIncidentV1; replayed: boolean }> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (current) {
        ...
        await tx.query(`UPDATE control_service_incidents SET severity=$4,safe_reason_code=$5,last_observed_at=$6 WHERE tenant_id=$1 AND correlation_key=$2 AND generation=$3`, [input.tenantId,input.correlationKey,prior.generation,severity,input.safeReasonCode,input.observedAt]);
        return { incident: { ...prior, severity, safeReasonCode: input.safeReasonCode, lastObservedAt: input.observedAt }, replayed: true };
      }
      ...
      const incident: ServiceIncidentV1 = { id: `incident:${input.tenantId}:${input.correlationKey}:${generation}`, tenantId: input.tenantId, correlationKey: input.correlationKey, generation, serviceId: input.serviceId, severity: input.severity, safeReasonCode: input.safeReasonCode, state: "open", openedAt: input.observedAt, lastObservedAt: input.observedAt };
      await tx.query(`INSERT INTO control_service_incidents (id,tenant_id,correlation_key,generation,service_id,severity,safe_reason_code,state,opened_at,last_observed_at) VALUES ($1,$2,$3,$4,$5,$6,$7,'open',$8,$8)`, [incident.id,incident.tenantId,incident.correlationKey,incident.generation,incident.serviceId,incident.severity,incident.safeReasonCode,incident.openedAt]);
...
interface Row { id: string; tenant_id: string; correlation_key: string; generation: number; service_id: string; severity: ServiceIncidentV1["severity"]; safe_reason_code: string; state: ServiceIncidentV1["state"]; opened_at: string | Date; last_observed_at: string | Date; resolved_at: string | Date | null; }
function toInstant(value: string | Date): string { return new Date(value).toISOString(); }
function rowToIncident(row: Row): ServiceIncidentV1 { return { id: row.id, tenantId: row.tenant_id, correlationKey: row.correlation_key, generation: Number(row.generation), serviceId: row.service_id, severity: row.severity, safeReasonCode: row.safe_reason_code, state: row.state, openedAt: toInstant(row.opened_at), lastObservedAt: toInstant(row.last_observed_at), ...(row.resolved_at ? { resolvedAt: toInstant(row.resolved_at) } : {}) }; }
