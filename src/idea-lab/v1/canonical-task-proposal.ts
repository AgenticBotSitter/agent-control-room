...
export class IdeaLabCanonicalTaskProposalServiceV1 {
  constructor(private readonly tasks: Pick<WebTaskService, "proposeWithDependencies">,
  ...
  async proposeRound(identity: VerifiedWebIdentity, input: {
    ...
    const linkedJobs = new Map((await this.links.list(session.tenantId, session.sessionId))
      .map((link) => [link.taskKey, link.jobId] as const));
    ...
    for (const plan of plans) {
      ...
      const dependencyJobIds = plan.dependsOnTaskKeys.map((taskKey) => linkedJobs.get(taskKey));
      if (dependencyJobIds.some((jobId) => !jobId)) throw new IdeaLabErrorV1("state_conflict");
      const result = await this.tasks.proposeWithDependencies(identity, this.scope.projectId, plan.taskDraft,
        `idea-round-proposal:${plan.planDigest.slice(7)}`, dependencyJobIds as string[]);
      await this.links.record(plan, result.receipt);
