...
export class IdeaLabBotRunStoreV1 {
  ...
  async #append(runId:string,change:(current:IdeaLabBotRunV1)=>IdeaLabBotRunV1):Promise<IdeaLabBotRunV1>{return this.#db.transaction(async tx=>{const rows=await tx.query<Row>(`SELECT version,payload,run_digest,run_auth_tag FROM control_idea_bot_run_events WHERE run_id=$1 ORDER BY version DESC LIMIT 1 FOR UPDATE`,[runId]);if(!rows.rows[0])throw new IdeaLabErrorV1("not_found");const current=this.#verify(rows.rows[0]),next=change(current),version=Number(rows.rows[0].version)+1;
  ...
  async complete(runId:string,at:string){return this.#append(runId,current=>{if(current.state!=="running"||current.attempts.at(-1)?.state!=="completed")throw new IdeaLabErrorV1("state_conflict");return buildIdeaLabBotRunV1({runId:current.runId,tenantId:current.tenantId,workspaceId:current.workspaceId,sessionId:current.sessionId,sessionDigest:current.sessionDigest,evidenceDigests:current.evidenceDigests,state:"completed",attempts:current.attempts,messagesUsed:current.messagesUsed,costUsd:current.costUsd,safeCode:"panel_completed",providerContacted:current.providerContacted,startedAt:current.startedAt,updatedAt:at});});}
  async recover(runId:string,at:string){const current=await this.get(runId);if(!current)throw new IdeaLabErrorV1("not_found");if(current.state!=="running")return current;const prior=current.attempts.at(-1);if(!prior||prior.state!=="provider_marked")throw new IdeaLabErrorV1("integrity_failed");return this.markAmbiguous(runId,prior.attemptId,"restart_after_provider_marker",at);}
}
