 ...
 * Test and local-run use only. The `session` argument is accepted for
 * interface compatibility and ignored: each method is atomic, and
 * cross-method races resolve through the same fail-closed outcomes as the
 * database guards (insert conflict on either unique key; swap mismatch on
 * stale prior changes nothing).
 */
export class InMemoryNeutralReservationPort implements NeutralReservationPort {
  private readonly records = new Map<string, NeutralReservationRowV1>();
  private readonly artifactIndex = new Map<string, string>();
