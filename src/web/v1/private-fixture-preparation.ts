...
import { privateWebMigrationRange, privateWebSchemaDigest, privateWebTableCount,
  readPrivateWebSchemaDigest } from "./private-database-preflight";
...
const packetSchema = z.object({ manifest: manifestSchema, migratorScopeDigest: digest, webScopeDigest: digest,
  ...
  migrationsApplied: z.literal(privateWebMigrationRange), setupRolesAccepted: z.literal(true),
  ...
  return Object.freeze({ async prepare(input: FixturePreparationInput): Promise<FixturePreparationResult> {
    ...
    try {
      ...
      await Promise.race([pool.client.transactionWithPreCommitCheck(async original => {
        ...
        ensure(tables.length === privateWebTableCount && tables.every(table => /^[a-z][a-z0-9_]{0,62}$/.test(table.name)));
