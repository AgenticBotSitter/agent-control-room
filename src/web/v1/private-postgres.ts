...
export function validatePrivatePostgresConfiguration(input: PrivatePostgresConfiguration) {
  ...
  input = captured as unknown as PrivatePostgresConfiguration;
  if (input.majorVersion !== 17 || !Number.isInteger(input.port)
    || input.port < 1 || input.port > 65535 || typeof input.database !== "string"
    || !/^[a-z][a-z0-9_]{0,62}$/.test(input.database) || typeof input.username !== "string"
