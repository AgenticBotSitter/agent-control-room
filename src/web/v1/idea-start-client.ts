...
import { ideaStartDraftSchema, ideaStartReceiptSchema } from "./idea-wire";
import { catalogProjectIdSchema } from "./project-wire";

/** One explicit request, never an automatic retry. The server owns the durable run claim. */
export function createIdeaStartClient(transport: typeof fetch = fetch) {
  let busy = false;
  return { async start(sessionId: string, value: unknown) {
    const input = ideaStartDraftSchema.safeParse(value);
    ...
    try {
      const response = await transport(`/api/v1/ideas/${encodeURIComponent(sessionId)}/start`, {
      ...
      const receipt = ideaStartReceiptSchema.parse(await readBrowserJson(response));
      if (receipt.sessionId !== sessionId || receipt.sessionDigest !== input.data.sessionDigest
        || receipt.runId !== `idea-run:${input.data.sessionDigest.slice(7, 31)}`) throw new Error();
      return receipt;
