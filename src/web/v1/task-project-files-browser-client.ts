...
export async function readTaskProjectFiles(projectId: string, transport: typeof fetch = fetch, signal?: AbortSignal) {
  try {
    if (!catalogProjectIdSchema.safeParse(projectId).success) throw new BrowserRequestError("invalid_request");
    signal?.throwIfAborted();
    const response = await transport(`/api/v1/projects/${encodeURIComponent(projectId)}/files`, {
    ...
    const value = taskProjectFilesSchema.parse(await readBrowserJson(response));
