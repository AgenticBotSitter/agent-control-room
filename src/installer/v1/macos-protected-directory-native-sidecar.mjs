...
/**
 * This is a separately-versioned macOS sidecar for the protected-directory
 ...
 */
export const MACOS_PROTECTED_DIRECTORY_NATIVE_SIDECAR_V1 =
  "control-room.macos-protected-directory-native-sidecar/v1";
export const PROTECTED_DIRECTORY_NATIVE_ARTIFACT_V1 =
  "control-room.protected-directory-native-artifact/v1";
export const PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1 = "PROTECTED_DIRECTORY_MANIFEST.json";
export const MACOS_PROTECTED_DIRECTORY_SIDECAR_MANIFEST_NAME_V1 = "MACOS_PROTECTED_DIRECTORY_SIDECAR.json";
const CHECKSUM_NAME = "SHA256SUMS";
const EXECUTABLE_NAME = "protected-directory-v1";
export const PROTECTED_DIRECTORY_NATIVE_REVIEWED_CFLAGS_V1 = Object.freeze([
...
const refused = () => { throw new Error("macos_protected_directory_native_sidecar_refused"); };
...
function parseArtifactManifest(bytes) {
  ...
  if (value.schema !== PROTECTED_DIRECTORY_NATIVE_ARTIFACT_V1 || value.platform !== "darwin"
    || !["arm64", "x64"].includes(value.architecture) || minimumMacos(value.minimumMacos) !== "13.0"
    || value.protocol !== "ACRDIR1" || !digestPattern.test(value.sourceSha256) || value.ownerQualified !== false) refused();
  ...
  if (typeof value.toolchain.compiler !== "string" || !/^Apple clang version [0-9][\x20-\x7e]{1,160}$/u.test(value.toolchain.compiler)
    ...
    || JSON.stringify(value.toolchain.flags) !== JSON.stringify(PROTECTED_DIRECTORY_NATIVE_REVIEWED_CFLAGS_V1)) refused();
...
function parseArtifactArchive(bytes, artifact, manifestBytes) {
  ...
  const root = `agent-control-room-protected-directory-darwin-${artifact.architecture}`;
  const expectedFiles = new Set([...artifact.files.map(file => file.path), PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1]);
  ...
  while (offset + 512 <= tar.length) {
    ...
    if (directory) {
      ...
      const expectedMode = entry ? Number.parseInt(entry.mode, 8) : path === PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1 ? 0o644 : undefined;
  ...
  if (!files.get(PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1)?.equals(manifestBytes)) refused();
...
async function artifactDirectory(directoryInput, allowsSidecarManifest = false) {
  ...
  const required = [PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1, CHECKSUM_NAME,
    ...(allowsSidecarManifest ? [MACOS_PROTECTED_DIRECTORY_SIDECAR_MANIFEST_NAME_V1] : [])].sort();
  ...
  if (!/^agent-control-room-protected-directory-darwin-(?:arm64|x64)\.tar\.gz$/u.test(archiveName)) refused();
  const [manifestStat, checksumStat, archiveStat] = await Promise.all([
    regular(join(directory, PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1), directory, 64 * 1024),
  ...
  const manifestBytes = await readFile(join(directory, PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1));
  const artifact = parseArtifactManifest(manifestBytes);
  if (archiveName !== `agent-control-room-protected-directory-darwin-${artifact.architecture}.tar.gz`) refused();
...
function artifactSummary(verified) {
  return Object.freeze({ schema: PROTECTED_DIRECTORY_NATIVE_ARTIFACT_V1, platform: verified.artifact.platform,
...
export async function verifyProtectedDirectoryNativeArtifactV1(directory) {
  return artifactSummary(await artifactDirectory(directory));
}

export async function buildMacosProtectedDirectoryNativeSidecarManifestV1(input) {
  const captured = exact(input, ["artifactDirectory", "releaseVersion"]);
  const releaseVersion = version(captured.releaseVersion), verified = await verifyProtectedDirectoryNativeArtifactV1(captured.artifactDirectory);
  return sidecarManifest(releaseVersion, verified);
}

function sidecarManifest(releaseVersion, verified) {
  return Object.freeze({ schema: MACOS_PROTECTED_DIRECTORY_NATIVE_SIDECAR_V1, releaseVersion, platform: "darwin",
...
function parseSidecarManifest(bytes) {
  ...
  if (value.schema !== MACOS_PROTECTED_DIRECTORY_NATIVE_SIDECAR_V1 || value.platform !== "darwin" || !["arm64", "x64"].includes(value.architecture)
    || minimumMacos(value.minimumMacos) !== "13.0" || value.protocol !== "ACRDIR1" || !digestPattern.test(value.sourceSha256)
  ...
  if (value.archiveName !== `agent-control-room-protected-directory-darwin-${value.architecture}.tar.gz`) refused();
  const artifact = { schema: PROTECTED_DIRECTORY_NATIVE_ARTIFACT_V1, platform: value.platform, architecture: value.architecture,
...
async function verifySidecarInternal(sidecarRootInput, expectedInput = {}) {
  ...
  const sidecarPath = join(sidecarRoot, MACOS_PROTECTED_DIRECTORY_SIDECAR_MANIFEST_NAME_V1);
  ...
  const summary = Object.freeze({ schema: MACOS_PROTECTED_DIRECTORY_NATIVE_SIDECAR_V1, verified: true, releaseVersion: sidecar.releaseVersion,
...
export async function verifyMacosProtectedDirectoryNativeSidecarV1(sidecarRootInput, expected = {}) {
 ...
 * object is the only compatible input shape for the existing protected-root factory.
 */
export async function stageMacosProtectedDirectoryNativeFactoryInputV1(input) {
  const captured = exact(input, ["sidecarRoot", "stagingParent"]);
  if (typeof captured.sidecarRoot !== "string" || typeof captured.stagingParent !== "string") refused();
  ...
  const staging = await mkdtemp(join(parent, ".acr-protected-directory-sidecar-"));
  try {
    ...
    return Object.freeze({ schema: MACOS_PROTECTED_DIRECTORY_NATIVE_SIDECAR_V1, nativeArtifact: sidecar,
      protectedRootNativeFactoryInput: Object.freeze({ executablePath, executableSha256: sidecar.executableSha256 }),
...
export async function copyVerifiedMacosProtectedDirectoryNativeSidecarV1(input) {
  ...
  try {
    ...
    await writeFile(join(destination, PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1), artifact.manifestBytes, { flag: "wx", mode: 0o644 });
    ...
    for (const name of [PROTECTED_DIRECTORY_NATIVE_MANIFEST_NAME_V1, CHECKSUM_NAME, sidecar.archiveName]) await chmod(join(destination, name), 0o644);
    await writeFile(join(destination, MACOS_PROTECTED_DIRECTORY_SIDECAR_MANIFEST_NAME_V1), canonicalBytes(sidecar), { flag: "wx", mode: 0o644 });
    await chmod(join(destination, MACOS_PROTECTED_DIRECTORY_SIDECAR_MANIFEST_NAME_V1), 0o644);
    return verifyMacosProtectedDirectoryNativeSidecarV1(destination, { releaseVersion });
