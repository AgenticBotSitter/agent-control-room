...
export async function preparePrivateLocalHermesAdmissionRequestV1(inputValue: unknown,
  ...
  const input = exact(inputValue, ["admissionPreparationInput", "privateStartupConfiguration", "startupAdmissionBinding"]);
...
function captureRuntime(value: unknown) {
  const runtime = exact(value, ["journal", "signal", "controlDeadlineMs", "confirmOwnerAttachedTerminal"]);
  ...
  if (!runtime.signal || typeof runtime.signal !== "object"
    ...
    || (runtime.controlDeadlineMs as number) > 30_000 || typeof runtime.confirmOwnerAttachedTerminal !== "function") return refused();
...
export async function runPrivateLocalHermesAdmissionV1(input: unknown,
  ...
  try {
    ...
    return Object.freeze({ schema: PRIVATE_LOCAL_HERMES_ADMISSION_RUNNER_V1,
      installationId: request.installationId, requestDigest: request.requestDigest,
      disposition: "terminal_confirmation" as const, terminalConfirmation,
      invokesHermes: false as const, startsService: false as const, startsWork: false as const,
