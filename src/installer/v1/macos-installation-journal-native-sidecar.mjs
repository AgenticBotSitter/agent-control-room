...
/**
 * This is a separately-versioned macOS sidecar for the installation-journal
...
export const MACOS_INSTALLATION_JOURNAL_NATIVE_SIDECAR_V1 =
  "control-room.macos-installation-journal-native-sidecar/v1";
export const INSTALLATION_JOURNAL_NATIVE_ARTIFACT_V1 =
  "control-room.installation-journal-native-artifact/v1";
export const INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1 = "INSTALLATION_JOURNAL_MANIFEST.json";
export const MACOS_INSTALLATION_JOURNAL_SIDECAR_MANIFEST_NAME_V1 = "MACOS_INSTALLATION_JOURNAL_SIDECAR.json";
const CHECKSUM_NAME = "SHA256SUMS";
const EXECUTABLE_NAME = "installation-journal-session-v1";
export const INSTALLATION_JOURNAL_NATIVE_REVIEWED_CFLAGS_V1 = Object.freeze([
...
const refused = () => { throw new Error("macos_installation_journal_native_sidecar_refused"); };
...
function parseArtifactManifest(bytes) {
  ...
  if (value.schema !== INSTALLATION_JOURNAL_NATIVE_ARTIFACT_V1 || value.platform !== "darwin"
    || !["arm64", "x64"].includes(value.architecture) || minimumMacos(value.minimumMacos) !== "13.0"
    || value.protocol !== "ACRJNL1" || !digestPattern.test(value.sourceSha256) || value.ownerQualified !== false) refused();
  ...
  if (typeof value.toolchain.compiler !== "string" || !/^Apple clang version [0-9][\x20-\x7e]{1,160}$/u.test(value.toolchain.compiler)
    ...
    || JSON.stringify(value.toolchain.flags) !== JSON.stringify(INSTALLATION_JOURNAL_NATIVE_REVIEWED_CFLAGS_V1)) refused();
...
function parseArtifactArchive(bytes, artifact, manifestBytes) {
  ...
  const root = `agent-control-room-installation-journal-darwin-${artifact.architecture}`;
  const expectedFiles = new Set([...artifact.files.map(file => file.path), INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1]);
  ...
  while (offset + 512 <= tar.length) {
    ...
    if (directory) {
      ...
      const expectedMode = entry ? Number.parseInt(entry.mode, 8) : path === INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1 ? 0o644 : undefined;
  ...
  if (!files.get(INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1)?.equals(manifestBytes)) refused();
...
async function artifactDirectory(directoryInput, allowsSidecarManifest = false) {
  ...
  const required = [INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1, CHECKSUM_NAME,
    ...(allowsSidecarManifest ? [MACOS_INSTALLATION_JOURNAL_SIDECAR_MANIFEST_NAME_V1] : [])].sort();
  ...
  if (!/^agent-control-room-installation-journal-darwin-(?:arm64|x64)\.tar\.gz$/u.test(archiveName)) refused();
  const [manifestStat, checksumStat, archiveStat] = await Promise.all([
    regular(join(directory, INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1), directory, 64 * 1024),
  ...
  const manifestBytes = await readFile(join(directory, INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1));
  const artifact = parseArtifactManifest(manifestBytes);
  if (archiveName !== `agent-control-room-installation-journal-darwin-${artifact.architecture}.tar.gz`) refused();
...
function artifactSummary(verified) {
  return Object.freeze({ schema: INSTALLATION_JOURNAL_NATIVE_ARTIFACT_V1, platform: verified.artifact.platform,
...
export async function verifyInstallationJournalNativeArtifactV1(directory) {
  return artifactSummary(await artifactDirectory(directory));
}

export async function buildMacosInstallationJournalNativeSidecarManifestV1(input) {
  const captured = exact(input, ["artifactDirectory", "releaseVersion"]);
  const releaseVersion = version(captured.releaseVersion), verified = await verifyInstallationJournalNativeArtifactV1(captured.artifactDirectory);
  return sidecarManifest(releaseVersion, verified);
}

function sidecarManifest(releaseVersion, verified) {
  return Object.freeze({ schema: MACOS_INSTALLATION_JOURNAL_NATIVE_SIDECAR_V1, releaseVersion, platform: "darwin",
...
function parseSidecarManifest(bytes) {
  ...
  if (value.schema !== MACOS_INSTALLATION_JOURNAL_NATIVE_SIDECAR_V1 || value.platform !== "darwin" || !["arm64", "x64"].includes(value.architecture)
    || minimumMacos(value.minimumMacos) !== "13.0" || value.protocol !== "ACRJNL1" || !digestPattern.test(value.sourceSha256)
  ...
  if (value.archiveName !== `agent-control-room-installation-journal-darwin-${value.architecture}.tar.gz`) refused();
  const artifact = { schema: INSTALLATION_JOURNAL_NATIVE_ARTIFACT_V1, platform: value.platform, architecture: value.architecture,
...
function macosVersionFromKernel(value) {
  ...
  // Darwin 22 is macOS 13. Earlier mappings cannot meet this sidecar's minimum.
...
async function verifySidecarInternal(sidecarRootInput, expectedInput = {}) {
  ...
  const sidecarPath = join(sidecarRoot, MACOS_INSTALLATION_JOURNAL_SIDECAR_MANIFEST_NAME_V1);
...
export async function verifyMacosInstallationJournalNativeSidecarV1(sidecarRootInput, expected = {}) {
...
export async function stageMacosInstallationJournalNativeFactoryInputV1(input) {
  ...
  try {
    ...
    return Object.freeze({ schema: MACOS_INSTALLATION_JOURNAL_NATIVE_SIDECAR_V1, nativeArtifact: sidecar,
      installationJournalNativeFactoryInput: Object.freeze({ executablePath, executableSha256: sidecar.executableSha256 }),
...
export async function copyVerifiedMacosInstallationJournalNativeSidecarV1(input) {
  ...
  try {
    ...
    await writeFile(join(destination, INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1), artifact.manifestBytes, { flag: "wx", mode: 0o644 });
    ...
    for (const name of [INSTALLATION_JOURNAL_NATIVE_MANIFEST_NAME_V1, CHECKSUM_NAME, sidecar.archiveName]) await chmod(join(destination, name), 0o644);
    await writeFile(join(destination, MACOS_INSTALLATION_JOURNAL_SIDECAR_MANIFEST_NAME_V1), canonicalBytes(sidecar), { flag: "wx", mode: 0o644 });
    await chmod(join(destination, MACOS_INSTALLATION_JOURNAL_SIDECAR_MANIFEST_NAME_V1), 0o644);
    return verifyMacosInstallationJournalNativeSidecarV1(destination, { releaseVersion });
