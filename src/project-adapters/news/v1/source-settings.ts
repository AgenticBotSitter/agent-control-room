...
import { newsDiscoveryEndpointSchemaV1 as url } from "./schemas";
...
export class PostgresNewsSourceSettings {
  ...
  private tag(payload: RecordValue) { return hmacSha256Tag(this.key, { kind: "news-source-setting/v1", ...this.scope, payload }); }
  ...
  private async read(db: DatabaseSession, sourceId: string) {
    const row = (await db.query<Row>(`SELECT source_id,revision,payload,auth_tag FROM control_news_source_settings
  ...
  async list(after?: string) {
    ...
    const rows = (await this.db.query<Row>(`SELECT DISTINCT ON (source_id COLLATE "C") source_id,revision,payload,auth_tag
      FROM control_news_source_settings WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3
  ...
  async save(value: unknown, expectedRevision: unknown, updatedAt: unknown) {
    ...
    return this.db.transaction(async tx => {
      ...
      await tx.query(`INSERT INTO control_news_source_settings (tenant_id,workspace_id,project_id,source_id,revision,payload,auth_tag)
