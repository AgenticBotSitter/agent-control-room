...
export type DiscoveryBatch = { stories: NewsStoryV1[]; state: "available" | "partial";
...
export async function saveNewsDiscovery(db: DatabaseClient,
  ...
  saveBaseline?: (tx: DatabaseSession) => Promise<void>) {
    ...
    return db.transaction(async tx => {
      ...
      const store = new PostgresNewsStoreV1(joined(tx), { tenantId, workspaceId, projectId }, key);
