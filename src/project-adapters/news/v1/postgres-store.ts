...
import { buildNewsWorkOrderProposalV1, parseNewsWorkOrderProposalV1 } from "./proposal";
import type { NewsStoryV1, NewsWorkOrderProposalV1 } from "./types";
...
export const newsDiscoveryCollectionLimit = 100_000;
const collectionReceiptSchema = scopeSchema.extend({ schema: z.literal("control-room.news-collection-receipt/v1"),
  sourceId: id, statusDigest: digestSchema,
  stories: z.array(z.object({ storyId: id, storyDigest: digestSchema }).strict()).max(newsDiscoveryCollectionLimit),
...
export class PostgresNewsStoreV1 {
  ...
  private story(row: Row): NewsStoryV1 {
    const value = parseNewsStoryV1(row.payload); this.check(value);
    if (this.tag("story", value) !== row.auth_tag) throw new ProjectWorkspaceContractErrorV1("integrity_failed");
    return value;
  }
  private proposal(row: Row): NewsWorkOrderProposalV1 {
    const value = parseNewsWorkOrderProposalV1(row.payload); this.check(value);
  ...
  async saveStory(value: unknown) {
    const story = parseNewsStoryV1(value); this.check(story);
    return this.db.transaction(async tx => {
      await this.project(tx);
      const inserted = await tx.query(`INSERT INTO control_news_story_versions
      ...
      const row = (await tx.query<Row>(`SELECT payload,auth_tag FROM control_news_story_versions
  ...
  async saveStories(values: unknown) {
    ...
    const stories = values.map(value => { const story = parseNewsStoryV1(value); this.check(story); return story; });
    return this.db.transaction(async tx => {
      ...
      const store = new PostgresNewsStoreV1(joined, this.scope, this.key, this.clock);
  ...
  async saveSourceStatus(value: unknown) {
    ...
    return this.db.transaction(async tx => {
      await this.project(tx);
      const inserted = await tx.query(`INSERT INTO control_news_source_observations
      ...
      const row = (await tx.query<Row & { source_id: string; status_digest: string; checked_at: Date | string }>(
        `SELECT payload,auth_tag,source_id,status_digest,checked_at FROM control_news_source_observations
  ...
  async listSourceStatuses(after?: string) {
    ...
    const rows = await this.db.query<Row & { source_id: string; status_digest: string; checked_at: Date | string }>(
      `SELECT DISTINCT ON (source_id COLLATE "C") source_id,status_digest,checked_at,payload,auth_tag
       FROM control_news_source_observations WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3
  ...
  async getSourceStatus(sourceId: string, digest?: string) {
    ...
    const row = (await this.db.query<Row & { source_id: string; status_digest: string; checked_at: Date | string }>(
      `SELECT source_id,status_digest,checked_at,payload,auth_tag FROM control_news_source_observations
  ...
  async saveCollection(values: unknown, statusValue: unknown) {
    ...
    if (!Array.isArray(values) || values.length > newsDiscoveryCollectionLimit) throw new ProjectWorkspaceContractErrorV1("invalid_input");
    const stories = values.map(value => { const story = parseNewsStoryV1(value); this.check(story); return story; });
    ...
    return this.db.transaction(async tx => {
      ...
      const payload = { schema: "control-room.news-collection-receipt/v1" as const, ...this.scope,
  ...
  async listStories(after?: string, filter: { view: "all" | "history" | "archive" | "fresh"; observedAt: string;
    ...
    const rows = await this.db.query<Row & { story_id: string; story_digest: string }>(`WITH latest AS (SELECT DISTINCT ON (story_id COLLATE "C") story_id,story_digest,payload,auth_tag
      FROM control_news_story_versions WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3
  ...
  async getStory(storyId: string, digest?: string) {
    ...
    const row = (await this.db.query<Row & { story_digest: string }>(`SELECT payload,auth_tag,story_digest
      FROM control_news_story_versions WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3 AND story_id=$4
  ...
  async getProposal(proposalId: string) {
    ...
    const row = (await this.db.query<Row & { proposal_digest: string; story_id: string; story_digest: string; idempotency_key: string }>(
      `SELECT payload,auth_tag,proposal_digest,story_id,story_digest,idempotency_key FROM control_news_research_proposals
  ...
  async saveProposal(value: unknown) {
    const proposal = parseNewsWorkOrderProposalV1(value); this.check(proposal);
    return this.db.transaction(async tx => {
      await this.project(tx);
      const source = (await tx.query<Row>(`SELECT payload,auth_tag FROM control_news_story_versions
        WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3 AND story_id=$4 AND story_digest=$5`,
      ...
      const rebuilt = buildNewsWorkOrderProposalV1({ ...this.scope, story, proposalId: proposal.proposalId,
      ...
      const inserted = await tx.query(`INSERT INTO control_news_research_proposals
      ...
      const rows = await tx.query<Row>(`SELECT payload,auth_tag FROM control_news_research_proposals
