...
import { buildNewsFeedProposedWork, verifyNewsFeedJobPlan } from "./feed-job-plan";

type Work = ReturnType<typeof buildNewsFeedProposedWork>;
const scopeSchema = z.object({ tenantId: localId, workspaceId: localId, projectId: localId }).strict();
type Scope = z.infer<typeof scopeSchema>;
const fail = (): never => { throw new Error("news_feed_plan_unavailable"); };
...
function parseWork(value: unknown): Work {
  ...
  try {
    const rebuilt = buildNewsFeedProposedWork({ plan: work.plan, requestId: work.request.id, workflowId: work.workflow.id,
...
export class NewsFeedPlanStore {
  ...
  private tag(work: Work) { return hmacSha256Tag(this.key, { purpose: "news-feed-plan/v1", ...this.scope, work }); }
  ...
  private async readWith(tx: DatabaseSession, jobId: string): Promise<Work | null> {
    const row = (await tx.query<{ workspace_id: string; project_id: string; input_digest: string; payload: unknown; auth_tag: string }>(
      "SELECT workspace_id,project_id,input_digest,payload,auth_tag FROM control_news_feed_plans WHERE tenant_id=$1 AND job_id=$2",
    ...
    const job = jobRecordSchema.parse(current.payload); verifyNewsFeedJobPlan(job, work.plan);
  ...
  async saveProposed(value: unknown) {
    ...
    return this.db.transaction(async tx => {
      ...
      await tx.query("INSERT INTO control_news_feed_plans(tenant_id,workspace_id,project_id,job_id,input_digest,payload,auth_tag) VALUES($1,$2,$3,$4,$5,$6,$7)",
