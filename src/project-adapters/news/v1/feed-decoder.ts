...
import { newsCanonicalUrlSchemaV1 } from "./schemas";
...
export const newsFeedInputSchema = z.object({
  ...
  source: z.object({ sourceId: id, sourceLabel: label, sourceKind: z.enum(["rss", "atom"]),
    endpointUrl: newsCanonicalUrlSchemaV1 }).strict(),
...
export class NewsFeedDecodeError extends Error {
...
export async function decodeNewsFeed(value: unknown) {
  const parsed = newsFeedInputSchema.safeParse(value);
  if (!parsed.success) throw new NewsFeedDecodeError("invalid_feed");
  const { xml, source, observedAt, maxBytes, maxItems, ...scope } = parsed.data;
  const byteCount = Buffer.byteLength(xml, "utf8");
  if (byteCount > maxBytes) throw new NewsFeedDecodeError("feed_limit_exceeded");
  // RSS/Atom do not need DTDs. Reject declarations before invoking upstream XML parsing.
  if (/<!\s*(?:DOCTYPE|ENTITY)\b/i.test(xml) || xml.includes("\u0000")) throw new NewsFeedDecodeError("invalid_feed");
  let feed: Parser.Output<Parser.Item>;
  try { feed = await new Parser().parseString(xml); }
  catch { throw new NewsFeedDecodeError("invalid_feed"); }
  if (!Array.isArray(feed.items)) throw new NewsFeedDecodeError("invalid_feed");
  if (feed.items.length > maxItems) throw new NewsFeedDecodeError("feed_limit_exceeded");
  const feedDigest = sha256Digest({ xml });
  const stories: NewsStoryV1[] = [], urls = new Set<string>();
  ...
  for (const item of feed.items) {
    try {
      ...
      const canonicalUrl = newsCanonicalUrlSchemaV1.parse(canonicalizeNewsDiscoveredUrlV1(item.link));
      ...
      const story = buildNewsStoryV1({ ...scope, ...material,
        storyId: digestId("story.news", { ...scope, canonicalUrl }),
        clusterId: digestId("cluster.news", { canonicalUrl }), sourceLabel: source.sourceLabel,
        ...
        contentDigest, sourceEvidence: [{ evidenceId: digestId("evidence.news", evidence),
