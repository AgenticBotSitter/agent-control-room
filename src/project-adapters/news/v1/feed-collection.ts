...
import { newsFeedInputSchema } from "./feed-decoder";
import { NewsFeedIngestionService } from "./feed-ingestion";
import { createAbsPublicReader, type AbsPublicReaderPorts } from "./public-reader";

export const newsFeedCollectionConfigurationSchema = newsFeedInputSchema.omit({ xml: true, observedAt: true }).extend({ timeoutMs: z.number().int().min(1).max(30_000) });
type Receipt = Awaited<ReturnType<NewsFeedIngestionService["ingest"]>>;
...
export function createNewsFeedCollection(db: DatabaseClient, value: unknown, key: Uint8Array,
  source: AbsCurrentSourceAuthority, ports?: AbsPublicReaderPorts) {
  const { timeoutMs, ...config } = newsFeedCollectionConfigurationSchema.parse(value), endpoint = config.source.endpointUrl;
  ...
  const ingestion = new NewsFeedIngestionService(guarded, config, key);
