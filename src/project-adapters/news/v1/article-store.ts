...
export class PostgresArticleDetails {
  ...
  private tag(record: ArticleDetailRecord) { return hmacSha256Tag(this.key, { kind: "news-article-detail/v1", ...this.scope, payload: record }); }
  ...
  async get(storyId: string, storyDigest: string, detailDigest?: string) {
    ...
    const row = (await this.db.query<Row>(`SELECT payload,auth_tag,detail_digest FROM control_news_article_details
  ...
  async save(value: unknown) {
    ...
    return this.db.transaction(async tx => {
      ...
      const story = await new PostgresNewsStoreV1(joined, this.scope, this.key).getStory(record.storyId, record.storyDigest);
      if (!story || story.canonicalUrl !== record.canonicalUrl) throw new Error("news_article_source_mismatch");
      const inserted = await tx.query(`INSERT INTO control_news_article_details
