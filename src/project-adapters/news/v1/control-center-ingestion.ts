...
import { newsCanonicalUrlSchemaV1, newsDiscoveryEndpointSchemaV1 } from "./schemas";
import { canonicalizeNewsDiscoveredUrlV1 } from "./collection";
import { buildNewsStoryV1 } from "./story";
import type { NewsStoryV1 } from "./types";
import { saveNewsDiscovery } from "./discovery-ingestion";
import { discoverySnapshotSchema, readDiscoveryBaseline, saveDiscoveryBaseline } from "./discovery-baseline";
import { newsDiscoveryCollectionLimit } from "./postgres-store";
...
const configuration = z.object({ tenantId: id, workspaceId: id, projectId: id,
  source: z.object({ id, name: label, url: newsDiscoveryEndpointSchemaV1 }).strict() }).strict();
const resultSchema = z.object({ sourceUrl: newsDiscoveryEndpointSchemaV1, coverageComplete: z.boolean(), feedKind: z.enum(["rss", "atom"]).optional(),
  snapshot: discoverySnapshotSchema.optional(),
  status: z.object({ sourceId: id, source: label, mode: z.enum(["feed", "sitemap"]), endpoint: newsDiscoveryEndpointSchemaV1 }),
  items: z.array(z.object({ title: z.string().max(20_000), summary: z.string().max(20_000), url: z.string().max(2_000),
    publishedAt: z.string().max(100), discoveredAt: time.optional() })).max(newsDiscoveryCollectionLimit),
});
...
export class AbsControlCenterIngestion {
  ...
  async ingest(value: unknown, observedAt: unknown, previousBaseline?: unknown) {
    ...
    const stories: NewsStoryV1[] = [], seen = new Set<string>();
    let rejectedCount = 0, duplicateCount = 0;
    for (const item of result.items) {
      try {
        const canonicalUrl = newsCanonicalUrlSchemaV1.parse(canonicalizeNewsDiscoveredUrlV1(item.url));
        ...
        const story = buildNewsStoryV1({ ...scope, ...material,
          storyId: digestId("story.news", { ...scope, canonicalUrl }), clusterId: digestId("cluster.news", { canonicalUrl }),
          ...
          sourceEvidence: [{ evidenceId: digestId("evidence.news", evidence), sourceId: source.id, sourceKind,
    ...
    return saveNewsDiscovery(this.db, { ...scope, source: { sourceId: source.id, sourceLabel: source.name, sourceKind } },
