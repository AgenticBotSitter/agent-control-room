...
const extractedSchema = z.object({ status: z.literal("extracted"), sourceUrl: newsCanonicalUrlSchemaV1,
...
export async function readNewsArticleDetail(bindingValue: unknown, ports: {
  ...
  const checkStory = async () => {
    ...
    const story = parseNewsStoryV1(await ports.getStory(binding.storyId));
