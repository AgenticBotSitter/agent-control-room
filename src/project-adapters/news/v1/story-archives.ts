...
export class PostgresNewsStoryArchives {
  ...
  private tag(payload: ArchiveRecord) { return hmacSha256Tag(this.key, { kind: "news-story-archive/v1", ...this.scope, payload }); }
  ...
  private async read(tx: DatabaseSession, storyId: string) {
    const row = (await tx.query<NewsArchiveRow>(`SELECT story_id,revision,archived,payload,auth_tag FROM control_news_story_archives
  ...
  async snapshot() {
    ...
    for (;;) {
      const rows = (await this.db.query<NewsArchiveRow>(`SELECT DISTINCT ON (story_id COLLATE "C") story_id,revision,archived,payload,auth_tag
        FROM control_news_story_archives WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3
  ...
  async save(storyId: string, archived: boolean, expectedRevision: number, updatedAt: string) {
    ...
    return this.db.transaction(async tx => {
      ...
      if (!(await tx.query("SELECT story_id FROM control_news_story_versions WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3 AND story_id=$4 LIMIT 1", [...this.values(), storyId])).rows.length)
      ...
      if (!previous) {
        const count = (await tx.query<{ count: string }>(`SELECT count(DISTINCT story_id)::text AS count
          FROM control_news_story_archives WHERE tenant_id=$1 AND workspace_id=$2 AND project_id=$3`, this.values())).rows[0];
        if (Number(count.count) >= 100_000) throw new Error("news_archive_snapshot_limit");
      }
      await tx.query(`INSERT INTO control_news_story_archives(tenant_id,workspace_id,project_id,story_id,revision,archived,payload,auth_tag)
