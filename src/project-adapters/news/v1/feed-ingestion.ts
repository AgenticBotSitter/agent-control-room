...
import { newsFeedInputSchema, NewsFeedDecodeError, decodeNewsFeed } from "./feed-decoder";
import { saveNewsDiscovery } from "./discovery-ingestion";

const configuration = newsFeedInputSchema.omit({ xml: true, observedAt: true });
...
export class NewsFeedIngestionService {
  ...
  async ingest(xml: unknown, observedAt: unknown) {
    ...
    let decoded: Awaited<ReturnType<typeof decodeNewsFeed>>;
    try { decoded = await decodeNewsFeed({ ...this.config, xml, observedAt: checkedAt }); }
    catch (error) {
      if (!(error instanceof NewsFeedDecodeError)) throw error;
  ...
  private async save(checkedAt: string, decoded?: Awaited<ReturnType<typeof decodeNewsFeed>>, reason?: string) {
    return saveNewsDiscovery(this.db, this.config, this.key, checkedAt, decoded, reason);
  }
}
