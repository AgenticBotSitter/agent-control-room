...
import { newsFeedJobReferenceSchema } from "../../../persistence/pg-boss-news-feed-worker";
import { localId } from "../../../harness/v1/native-run-identifiers";
import { NewsFeedPlanStore } from "./feed-plan-store";
import { createNewsFeedCollection } from "./feed-collection";
...
export function createNewsFeedJobExecution(databases: { coordinator: DatabaseClient; ingestion: DatabaseClient }, value: unknown, key: Uint8Array,
  source: AbsCurrentSourceAuthority, clock: () => number = Date.now,
  factory: typeof createNewsFeedCollection = createNewsFeedCollection,
  discoveryPorts?: Required<Pick<PinnedFetchDependencies, "lookup" | "fetch">>) {
  const { coordinator: db, ingestion } = databases;
  if (!db || !ingestion || db === ingestion) throw new Error("news_feed_execution_database_separation_required");
  ...
  if (!(key instanceof Uint8Array) || key.length !== 32) throw new Error("news_feed_execution_config_invalid");
  ...
  const plans = new NewsFeedPlanStore(db, projectScope, capturedKey), store = new CanonicalStore(db);
  const news = new PostgresNewsStoreV1(ingestion, projectScope, capturedKey);
  ...
  return Object.freeze({ async collect(value: unknown, signal: AbortSignal) {
    ...
    if (!(signal instanceof AbortSignal) || signal.aborted || reference.tenantId !== scope.tenantId || reference.projectId !== scope.projectId)
      throw new Error("news_feed_execution_unavailable");
    const work = await plans.get(reference.jobId);
    if (!work || work.job.authority.allowedExecutor !== scope.executorId) throw new Error("news_feed_execution_unavailable");
    const discovery = work.plan.schema === "control-room.news-discovery-plan/v1" ? work.plan.configuration : undefined;
    if (discovery && !ports) throw new Error("news_feed_execution_unavailable");
    if (discovery) {
    ...
    assertSource(endpoint); if (signal.aborted) throw new Error("news_feed_execution_unavailable");
    const startedAt = clock();
    const marker = await store.beginNewsFeedAttempt({ ...scope, jobId: reference.jobId, attemptId: reference.attemptId,
    ...
    const current = (url: string = endpoint): undefined => {
      ...
      if (!Number.isSafeInteger(now) || now < startedAt || now >= deadline || signal.aborted || controller.signal.aborted)
        throw new Error("news_feed_execution_unavailable");
      if (discovery && !discovery.allowedOrigins.includes(new URL(url).origin + "/")) throw new Error("news_feed_execution_unavailable");
      assertSource(url);
    };
    ...
    let collection: ReturnType<typeof createNewsFeedCollection> | ReturnType<typeof createControlCenterCollection> | undefined;
    ...
    try {
      ...
      try {
        ...
        if (checked.status.sourceId !== expected.sourceId || checked.status.label !== expected.sourceLabel
          ...
          throw new Error("news_feed_result_mismatch");
        current();
      ...
      const result = await store.settleNewsFeedAttempt({ ...settlement, outcome, ...(receiptDigest ? { receiptDigest } : {}) }, clock);
