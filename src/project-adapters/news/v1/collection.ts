...
import { newsCollectionInputSchemaV1, newsFakeCollectionBatchSchemaV1 } from "./schemas";
import { buildNewsStoryV1 } from "./story";
import type {
  NewsCollectedItemV1,
  NewsCollectionResultV1,
  NewsFakeCollectionBatchV1,
  NewsSourceEvidenceV1,
  NewsStoryV1,
} from "./types";

const TRACKING_PARAMETER = /^(?:utm_[a-z0-9_]+|fbclid|gclid|mc_cid|mc_eid|ref|source)$/i;

export class NewsInjectedFakeCollectorV1 {
  readonly #batch: NewsFakeCollectionBatchV1;
  constructor(batchValue: unknown) {
    this.#batch = parseExactProjectWorkspaceV1(newsFakeCollectionBatchSchemaV1, batchValue) as NewsFakeCollectionBatchV1;
  }
  collect(): NewsFakeCollectionBatchV1 {
    return parseExactProjectWorkspaceV1(newsFakeCollectionBatchSchemaV1, this.#batch) as NewsFakeCollectionBatchV1;
  }
}
...
export function canonicalizeNewsDiscoveredUrlV1(value: string): string {
  let url: URL;
...
function preferred(items: readonly NewsCollectedItemV1[]): NewsCollectedItemV1 {
...
function evidence(item: NewsCollectedItemV1, canonicalUrl: string): NewsSourceEvidenceV1 {
  const material = {
  ...
  return {
    evidenceId: digestId("evidence.news", material),
    sourceId: item.sourceId,
    sourceKind: item.sourceKind,
    sourceLabel: item.sourceLabel,
    canonicalUrl,
...
export function collectNewsFakeSourcesV1(inputValue: unknown): NewsCollectionResultV1 {
  const input = parseExactProjectWorkspaceV1(newsCollectionInputSchemaV1, inputValue) as {
    tenantId: string; workspaceId: string; projectId: string; batches: NewsFakeCollectionBatchV1[]; collectedAt: string;
  };
  const scope = input;
  ...
  const all = batches.flatMap((batch) => batch.items).map((item) => ({ item, canonicalUrl: canonicalizeNewsDiscoveredUrlV1(item.discoveredUrl), titleKey: normalizedTitle(item.title) }));
  ...
  for (const [titleKey, entries] of [...clusters].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    const clusterId = digestId("cluster.news", { titleKey });
    stories.push(buildNewsStoryV1({
      storyId: digestId("story.news", { titleKey }),
