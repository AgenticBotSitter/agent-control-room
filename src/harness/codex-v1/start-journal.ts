...
type ReservationRow = ReservationRowV2 & { attempt_id: string };

function reservationMatchesV2(row: ReservationRowV2, admission: CodexStartAdmissionV1): boolean {
...
function reservationMatches(row: ReservationRow, admission: CodexStartAdmissionV1): boolean {
  return row.attempt_id === admission.scope.attemptId && reservationMatchesV2(row, admission);
}

function schemaTransaction(db: DatabaseSync, work: () => void): void {
  db.exec('BEGIN IMMEDIATE');
  try { work(); db.exec('COMMIT'); }
  catch (error) {
    try { db.exec('ROLLBACK'); } catch { /* constructor will quarantine the journal */ }
    throw error;
  }
}
      ...
      if (version === 0 && objects.length === 0) {
        ...
        schemaTransaction(this.db, () => {
          ...
          const insert = this.db.prepare(`INSERT INTO codex_start_reservations
            (run_id,attempt_id,queue_id,activation_id,activation_message_id,activation_digest,
             activation_frame_digest,dispatch_frame_digest,receipt_frame_digest,admission_id,
             connection_attempt_id,reserved_at,deadline,admission_json,admission_digest)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
          for (const row of rows) {
            const admission = verifyCodexStartAdmissionV1(JSON.parse(row.admission_json));
            if (!reservationMatchesV2(row, admission)) throw new Error('codex_start_journal_schema_invalid');
            insert.run(row.run_id, admission.scope.attemptId, row.queue_id, row.activation_id,
              row.activation_message_id, row.activation_digest, row.activation_frame_digest,
              row.dispatch_frame_digest, row.receipt_frame_digest, row.admission_id,
  ...
  reserveStart(value: unknown, reservedAtValue: string, assertCurrent: () => void): 'recorded' | 'duplicate' {
    ...
    return this.transaction(() => {
      ...
      this.db.prepare(`INSERT INTO codex_start_reservations
        (run_id,attempt_id,queue_id,activation_id,activation_message_id,activation_digest,activation_frame_digest,
        ...
        .run(admission.scope.runId, admission.scope.attemptId, admission.queueId, admission.activationId, admission.activationMessageId,
