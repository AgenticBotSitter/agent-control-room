...
const responseSchema = z.object({ thread: z.object({ id,
  turns: z.array(z.object({ id, status: z.enum(['inProgress', 'completed', 'interrupted', 'failed']) })).max(1024),
}) });
...
export function createCodexReadRecovery(input: z.infer<typeof bindingSchema>) {
  ...
  const request = Object.freeze({ method: 'thread/read' as const,
    params: Object.freeze({ threadId: binding.threadId, includeTurns: true as const }) });
