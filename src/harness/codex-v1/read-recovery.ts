...
import { projectExactPackageCodexCompletedTurnV1 } from './completed-turn';
  ...
  return Object.freeze({ request,
    project(rawResult: string) {
      try {
        ...
        let exactPackageResult: ReturnType<typeof projectExactPackageCodexCompletedTurnV1> | undefined;
        if (turn?.status === 'completed') {
          try { exactPackageResult = projectExactPackageCodexCompletedTurnV1({ ...binding, rawResult }); }
          catch { exactPackageResult = undefined; }
        }
        return Object.freeze({ ...binding, status: turn?.status ?? 'not_observed',
          ...
          ...(exactPackageResult ? { exactPackageResult } : {}) });
