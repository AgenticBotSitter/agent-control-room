...
export const harnessRunSchemaV1 = z.object({
  ...
  cancelState: z.enum(["not_requested", "requested", "confirmed", "unsupported"]), createdAt: time, updatedAt: time, startedAt: time.optional(), finishedAt: time.optional(), lastObservedAt: time, safeReasonCode: id.optional(),
  ...
  if (["running", "waiting_input", "waiting_approval", "cancelling", "succeeded", "failed", "cancelled"].includes(run.state) && !run.startedAt) context.addIssue({ code: "custom", message: "started lifecycle requires startedAt", path: ["startedAt"] });
});
