...
export class ReadyFrontierReadyPolicyStoreV1 {
  ...
  private verify(): { metadata: MetadataRowV1; policies: Array<{ row: PolicyRowV1; policy: ReadyFrontierReadyPolicyV1 }> } {
    const metadata = this.metadata();
  ...
  private stateDigest(rows: PolicyRowV1[]): string {
  ...
  private stateTag(revision: number, count: number, digest: string): string {
  ...
  private checkpoint(metadata: MetadataRowV1): RollbackCheckpointV1 {
