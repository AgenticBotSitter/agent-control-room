...
function snapshot(value: unknown, budget: BudgetV1, depth: number): unknown {
  if (depth > 24 || budget.remaining-- <= 0) throw new AgentTeamContractErrorV1("invalid_input");
  if (value === null || typeof value === "string" || typeof value === "boolean") return value;
  if (typeof value === "number" && Number.isFinite(value)) return value;
  if (!value || typeof value !== "object" || isHostProxyV1(value)) throw new AgentTeamContractErrorV1("invalid_input");
  if (Array.isArray(value)) {
    const items = exactHostDataArrayV1(value, 10_000);
    if (!items) throw new AgentTeamContractErrorV1("invalid_input");
    return items.map((item) => snapshot(item, budget, depth + 1));
  }
  if (Object.getPrototypeOf(value) !== Object.prototype) throw new AgentTeamContractErrorV1("invalid_input");
  const keys = Reflect.ownKeys(value);
  if (keys.length > 500 || keys.some((key) => typeof key !== "string")) throw new AgentTeamContractErrorV1("invalid_input");
...
export function exactAgentTeamJsonV1(value: unknown): unknown {
  return snapshot(value, { remaining: 75_000 }, 0);
}

export function parseExactAgentTeamV1<T>(schema: { parse(value: unknown): T }, value: unknown): T {
  let parsed: T;
  try {
  ...
  try {
    assertNoSecretMaterial(parsed, "agent team projection");
    assertSafeProjection(parsed);
  } catch {
    throw new AgentTeamContractErrorV1("redaction_rejected");
  }
  return parsed;
}
