import { z } from "zod";
import { nativeTaskSubmissionReferenceSchema, type NativeTaskSubmissionReference } from "./native-task-submission";
...
export async function startPgBossNativeTaskWorker(client: PgBossNativeWorkerClient, input: {
  concurrency?: number; deliver: NativeTaskDeliveryHandler;
}) {
  ...
  const id = await work(spec.name, { batchSize: 1, includeMetadata: true, localConcurrency: concurrency,
    ...
    try {
      ...
      let valid = parsed.success && job.name === spec.name && job.retryLimit === 0 && job.retryCount === 0
