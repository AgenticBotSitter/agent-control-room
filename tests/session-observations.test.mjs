...
import { herdrObservationPageSchema, herdrObservationFleetSchema } from '../src/web/v1/herdr-wire.ts';
...
const page = (projectId, ageMs = 0) => ({ projectId, status: 'online', ageMs,
  ...
  rows: [{ key: 'a'.repeat(64), status: 'done', duplicateSession: true, stateSource: 'advisory',
    executionAuthority: false, completionVerified: false, cleanupVerified: false }] });
const fleet = (projectId, ageMs = 0) => ({ projectId, sources: [{ sourceKey: 'b'.repeat(64), observation: page(projectId, ageMs) }] });
...
test('session panel isolates project changes, marks stale rows and hides observations on denied refresh', async () => {
  ...
  try {
    ...
    await act(async () => pending[0].resolve(Response.json(fleet('project:a'))));
    assert.doesNotMatch(text(), /completion unverified/);
    const mixed = fleet('project:b', 4900);
    mixed.sources.push({ sourceKey: 'c'.repeat(64), observation: { ...page('project:b'), status: 'offline', rows: [] } });
    await act(async () => pending[1].resolve(Response.json(mixed)));
    assert.match(text(), /1 of 2 enrolled observers/);
    ...
    assert.match(text(), /0 of 2 enrolled observers/);
    ...
    await act(async () => pending[3].resolve(Response.json(fleet('project:wrong'))));
  ...
  const group = fleet('project:a');
  assert.equal(herdrObservationFleetSchema.safeParse(group).success, true);
  assert.equal(herdrObservationFleetSchema.safeParse({ ...group, sources: [...group.sources, ...group.sources] }).success, false);
  assert.equal(herdrObservationFleetSchema.safeParse({ ...group, projectId: 'project:wrong' }).success, false);
});
