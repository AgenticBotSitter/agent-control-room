...
const base = (name = 'test', scope: { jobId?: string; runId?: string } = {}) => createCodexStartAdmissionV1({
  ...
  requestMessageId: `message:codex-start:${name}`, deliveryDigest: digest(`delivery:${name}`),
...
const threadResponse = (id = 10, threadId = 'thr_123') => JSON.stringify({ id,
  result: { approvalPolicy: 'never', approvalsReviewer: 'user', cwd: '/synthetic/project', model: 'model:test',
