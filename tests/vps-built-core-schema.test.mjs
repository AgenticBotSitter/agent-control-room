...
test("default test preparation retains the private external-content schema", async t => {
  ...
  const tables = await f.client.query("SELECT to_regclass('public.control_content_blooms_releases') AS present");
  assert.equal(tables.rows[0].present, "control_content_blooms_releases");
});
...
for (const profile of ["full", "without-external-content"]) test(`compiled schema boundary: ${profile}`, async t => {
  ...
  const tables = await f.client.query("SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename LIKE 'control_content_blooms%'");
