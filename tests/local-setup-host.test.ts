...
import { createInstalledLocalSetupHostV1, createLocalSetupHostV1, type InstalledLocalSetupHostRuntimeV1 } from "../src/installer/v1/local-setup-host";
...
type SetupBridge = Parameters<NonNullable<Parameters<typeof createLocalSetupHostV1>[0]["createService"]>>[0];

async function fixture() {
  let bridge: SetupBridge | undefined, closed = 0;
  const calls = { render: 0, plan: 0, readiness: 0 };
  const host = createLocalSetupHostV1({ origin: "http://127.0.0.1:3210", port: 3210,
    ...
    planSource: { async read() { calls.plan += 1; return { schema: LOCAL_SETUP_JOURNAL_SOURCE_V1, status: "available" as const,
      plan: createInstallationPlanViewV1(plan), restart: "ready_to_begin" as const, performsEffect: false as const, permitsRetry: false as const }; } },
  ...
  async function send(path: string, options: { method?: string; headers?: string[]; peer?: string; host?: string } = {}) {
    ...
    const completed = new Promise<void>((resolve, reject) => { (exchange.output as ServerResponse).once("finish", resolve); exchange.output.once("error", reject); });
...
test("generic local setup host exposes only page, redacted reads, and verified assets", async t => {
  ...
  assert.equal((await f.send("/_next/static/app.js")).body(), "asset"); assert.equal((await f.send("/favicon.svg")).body(), "icon");
  const head = await f.send("/setup", { method: "HEAD" }); assert.equal(head.output.statusCode, 200); assert.equal(head.body(), "");
  assert.equal((await f.send("/_next/static/app.js?x=1")).output.statusCode, 404); assert.equal((await f.send("/projects")).output.statusCode, 404);
  await f.host.close(); assert.equal(f.closed(), 1);
});

test("generic local setup transport refuses credentials, forwarding, writes and remote peers", async t => {
  const f = await fixture(); t.after(() => f.host.close());
  for (const headers of [["cookie", "owner=secret"], ["authorization", "Bearer secret"], ["x-forwarded-for", "127.0.0.1"], ["sec-fetch-site", "cross-site"]])
  ...
  for (const mode of ["malformed-plan", "plan-error", "readiness-error"] as const) {
    ...
    const host = createLocalSetupHostV1({ origin: "http://127.0.0.1:3210", port: 3210,
      ...
      isApplicationReady: () => true, async closeApplication() {}, createService(candidate) {
        bridge = candidate; return { isReady: candidate.isReady, async start() {}, close: candidate.close };
      } });
    if (!bridge) throw new Error("bridge_missing");
    ...
    const completed = new Promise<void>((resolve, reject) => { exchange.output.once("finish", resolve); exchange.output.once("error", reject); });
...
test("generic host construction is inert and exact-loopback-only", () => {
  let factories = 0;
  const base = { port: 3210, assets: { count: 2, digest: digest("assets"), respond: () => undefined }, render: () => new Response("setup"),
    planSource: { async read() { return { schema: LOCAL_SETUP_JOURNAL_SOURCE_V1, status: "unavailable" as const, performsEffect: false as const, permitsRetry: false as const }; } },
    isApplicationReady: () => true, async closeApplication() {}, createService(bridge: SetupBridge) { factories += 1; return { isReady: bridge.isReady, async start() {}, close: bridge.close }; } };
  createLocalSetupHostV1({ ...base, origin: "http://127.0.0.1:3210" }); assert.equal(factories, 1);
...
test("installed host selects the one canonical journal source without reading or writing at construction", () => {
  let journalReads = 0, sources = 0, services = 0;
  const runtime: InstalledLocalSetupHostRuntimeV1 = { createJournal(configuration) {
    assert.deepEqual(configuration, { rootDirectory: "/private/setup-journal", installationId: "local-setup-one", ownerUid: 501 });
    return { async inspectSettledHistory() { journalReads += 1; return []; } } as never;
  }, createJournalSource(journal) { sources += 1; return { async read() { await journal.inspectSettledHistory(); return { schema: LOCAL_SETUP_JOURNAL_SOURCE_V1,
    status: "unavailable" as const, performsEffect: false as const, permitsRetry: false as const }; } } as never; },
  createService(bridge) { services += 1; return { isReady: bridge.isReady, async start() {}, close: bridge.close }; } };
  createInstalledLocalSetupHostV1({ journalRoot: "/private/setup-journal", installationId: "local-setup-one", ownerUid: 501, port: 3210,
    assets: { count: 0, digest: digest("assets"), respond: () => undefined }, render: () => new Response("setup"),
    isApplicationReady: () => true, async closeApplication() {} }, runtime);
  assert.equal(sources, 1); assert.equal(services, 1); assert.equal(journalReads, 0);
});
