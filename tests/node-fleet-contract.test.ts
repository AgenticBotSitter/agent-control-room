...
test("fleet history is tenant-bound, append-only by sequence, and exact-replay safe", async () => {
  ...
  try {
    ...
    const capabilityOne = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:fleet", nodeId: "node:fleet", kind: "capability", source: "probe_runner", sequence: 1, observedAt: "2026-08-26T00:00:00.000Z", expiresAt: "2026-09-02T00:00:00.000Z", trust: "reported", fingerprint: digest, payload: { probeId: "probe.one", probeVersion: "1.0.0", outcome: "pass", reasonCode: "passed" } });
    const capabilityTwo = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:fleet", nodeId: "node:fleet", kind: "capability", source: "probe_runner", sequence: 2, observedAt: "2026-08-26T00:01:00.000Z", expiresAt: "2026-09-02T00:01:00.000Z", trust: "reported", fingerprint: digest, payload: { probeId: "probe.two", probeVersion: "1.0.0", outcome: "pass", reasonCode: "passed" } });
    await store.ingestAuthenticated(capabilityOne, capabilityOne.observedAt);
    await store.ingestAuthenticated(capabilityTwo, capabilityTwo.observedAt);
    assert.deepEqual((await store.current({ tenantId: "tenant:fleet", nodeId: "node:fleet" })).filter((item) => item.kind === "capability").map((item) => item.payload.probeId).sort(), ["probe.one", "probe.two"]);
    assert.deepEqual(
...
test("fleet eligibility reports stable, actionable reasons instead of guessing", () => {
  ...
  assert.deepEqual(evaluateFleetEligibility({ now: "2026-08-26T00:01:00.000Z", signals: [{ ...telemetry, observedAt: "2026-08-26T00:02:00.000Z", expiresAt: "2026-08-26T00:07:00.000Z" }], requiredScratchBytes: 1 }), { eligible: false, reasons: ["telemetry_stale"] });
  const discoverySignal = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:1", nodeId: "node:1", kind: "discovery", source: "static_collector", sequence: 1, observedAt: "2026-08-26T00:00:00.000Z", expiresAt: "2026-08-27T00:00:00.000Z", trust: "reported", fingerprint: digest, payload: discovery });
  const foreignCapability = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:1", nodeId: "node:2", kind: "capability", source: "probe_runner", sequence: 1, observedAt: "2026-08-26T00:00:00.000Z", expiresAt: "2026-08-27T00:00:00.000Z", trust: "reported", fingerprint: digest, payload: { probeId: "probe.gpu", probeVersion: "1.0.0", outcome: "pass", reasonCode: "passed" } });
  assert.deepEqual(
    evaluateFleetEligibility({ now: "2026-08-26T00:01:00.000Z", signals: [telemetry, foreignCapability], requiredScratchBytes: 1, requiredCapabilityProbeId: "probe.gpu" }),
    { eligible: false, reasons: ["signal_identity_mismatch"] },
  );
  const expiredCapability = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:1", nodeId: "node:1", kind: "capability", source: "probe_runner", sequence: 1, observedAt: "2026-08-26T00:00:00.000Z", expiresAt: "2026-08-26T00:00:30.000Z", trust: "reported", fingerprint: digest, payload: { probeId: "probe.gpu", probeVersion: "1.0.0", outcome: "pass", reasonCode: "passed" } });
  const mismatchedBenchmark = fleetSignalEnvelopeSchema.parse({ schemaVersion: "1.0.0", tenantId: "tenant:1", nodeId: "node:1", kind: "benchmark", source: "benchmark_runner", sequence: 1, observedAt: "2026-08-26T00:00:00.000Z", expiresAt: "2026-09-25T00:00:00.000Z", trust: "reported", fingerprint: digest, payload: { benchmarkId: "benchmark.render", benchmarkVersion: "1.0.0", workloadDigest: digest, outcome: "pass", reasonCode: "completed", normalizedScore: 1, scoreUnit: "points", environmentFingerprint: `sha256:${"b".repeat(64)}` } });
  assert.deepEqual(
    evaluateFleetEligibility({ now: "2026-08-26T00:01:00.000Z", signals: [telemetry, discoverySignal, expiredCapability, mismatchedBenchmark], requiredScratchBytes: 1, requiredCapabilityProbeId: "probe.gpu", requiredBenchmarkId: "benchmark.render" }),
    { eligible: false, reasons: ["capability_expired", "benchmark_environment_mismatch"] },
  );
});
