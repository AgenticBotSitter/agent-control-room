...
test("a local Hermes worker Hermes 0.21 template creates a pinned text-review plan, not an older generic Hermes plan", async t => {
  ...
  const authority: NativeTaskTemplate["authority"] = { projectId: binding.projectId, allowedExecutor: "executor:local-hermes",
    allowedOperations: [HERMES_021_MACOS_LOCAL_START_OPERATION_V1], credentialRefs: ["credential:local-hermes"], filesystemRoots: [],
  ...
  const template: NativeTaskTemplate = { id: "template:local-hermes-021", adapter: HERMES_021_MACOS_LOCAL_ADAPTER_V1, authority,
  ...
  const source = await f.tasks.propose(f.identity, binding.projectId, taskDraft, "local-hermes-021-plan-source");
  ...
  const telemetry: FleetSignalEnvelope = { schemaVersion: "1.0.0", tenantId: binding.tenantId, nodeId: binding.nodeId,
    sequence: await nextSignalSequence("telemetry"), observedAt: at(6000), expiresAt: at(120_000), trust: "reported", fingerprint: sha256Digest("local-hermes-telemetry"),
  ...
  const capability: FleetSignalEnvelope = { schemaVersion: "1.0.0", tenantId: binding.tenantId, nodeId: binding.nodeId,
    sequence: await nextSignalSequence("capability", HERMES_021_MACOS_LOCAL_CAPABILITY_V1), observedAt: at(6000), expiresAt: at(120_000), trust: "reported", fingerprint: sha256Digest("local-hermes-capability"),
  ...
  assert.equal(standardPickup.kind, "hermes-021-local", "the shared queue dispatcher selects local Hermes worker's local route");
  ...
  assert.equal(queuedReferences.length, 1);
  const localBinding = {
    localServiceId: "service:local-hermes-hermes", workerId: "worker:local-hermes", expectedVersion: "0.21.3" as const,
  ...
  const localExecutor = createHermes021LocalSubprocessQueueExecutorV1({ tenantId: binding.tenantId,
    ...
    host: { async execute(input) {
      launches++;
      await input.onLine(JSON.stringify({ type: "result", session_id: "session:local-hermes", exit_code: 0, text: "completed", tokens: {
  ...
  const revokedSource = await f.tasks.propose(f.identity, binding.projectId, taskDraft, "local-hermes-021-revocation-source");
  ...
  const workerClient: PgBossNativeWorkerClient = {
    ...
    async work(name, _options, handler) { assert.equal(name, PG_BOSS_NATIVE_SUBMISSION.name); queuedWorkerHandler = handler; return "worker:local-hermes"; },
    async cancel() {},
    async offWork(_name, input) { assert.deepEqual(input, { id: "worker:local-hermes", wait: true }); workerStopped++; },
  };
  ...
  assert.equal(launches, 1, "restart recovery never invokes local Hermes worker again");
