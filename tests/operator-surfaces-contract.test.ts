...
test("CR6E snapshot is tenant-bound, bounded, and rejects unsafe display material", () => {
  const snapshot = parseOperatorSurfaceSnapshotV1({
    ...
    portfolio: [{ projectId: "project:1", workflowCount: 1, activeJobCount: 1, waitingApprovalJobCount: 0, failedJobCount: 0, lastActivityAt: now }],
...
test("CR6E read projections are deterministic, keep failed delivery visible, and cannot elevate Owner Focus", () => {
  ...
  const snapshot = buildOperatorSurfaceSnapshotV1({ contractVersion: OPERATOR_SURFACES_CONTRACT_V1, tenantId: "tenant:1", generatedAt: now, fleet: [], bottlenecks: [], activeWork: [], portfolio: [], services: [], schedules: [], serviceIncidents: [], actionInbox: [openSoon, item], ownerFocus: [] });
