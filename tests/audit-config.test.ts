...
test("runtime configuration is fail-closed in production and summaries exclude secrets", () => {
  ...
  const config = loadRuntimeConfig({ NODE_ENV: "production", CONTROL_ROOM_WORKSPACE_ID: "workspace.prod", DATABASE_URL: "postgres://user:password@localhost/control", CONTROL_ROOM_PUBLIC_ORIGIN: "https://control.example", CONTROL_ROOM_SESSION_SECRET: "s".repeat(32) });
...
test("safe operational errors never expose internal error text", () => {
  assert.deepEqual(safeErrorResponse(new SafeOperationalError("forbidden", "Permission denied", "corr.1")), { code: "forbidden", message: "Permission denied", correlationId: "corr.1" });
