...
const local = { kind: "local" as const, workerId: "worker:local-hermes", adapterId: "connector:hermes-021-macos-local-v1", adapterRevision: "00570550" };
...
test("a this-computer plan preserves one authority and asks only for local proof", () => {
  ...
  assert.deepEqual(plan.retainedWorkerIds, ["worker:local-hermes"]);
...
test("moving a known worker between local and remote is a rebind, not quiet retention", () => {
  ...
  assert.deepEqual(plan.reboundWorkerIds, ["worker:local-hermes"]);
  assert.deepEqual(plan.addedRemoteWorkerIds, []);
  assert.deepEqual(plan.removedWorkerIds, []);
