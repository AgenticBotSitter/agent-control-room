...
async function prepared(storage = new ControlledStorage(), resultText = "Exact synthetic Codex result.") {
  ...
  const activationMaterial = { schema: "control-room.codex-task-activation/v1" as const,
    ...
    queueId: admission.queueId, connectionId: admission.connectionId,
    connection: { connectionAttemptId: "connection:result", initializedConnectionDigest: sha256Digest("initialized:result") },
