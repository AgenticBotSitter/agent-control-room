...
const binding = { localServiceId: "service:local-hermes", workerId: "worker:local-hermes", expectedVersion: "0.21.3" as const,
  sourceRevision: "00570550" };
const receivedAt = "2026-09-20T00:00:02.000Z";
const terminal = { type: "result" as const, session_id: "session:local-hermes", exit_code: 0, text: "bounded reviewed text",
...
function delivery() {
  return createControllerWorkerDeliveryV1({ identity: { tenantId: nativeBinding.tenantId, projectId: nativeBinding.projectId,
    jobId: nativeBinding.jobId, attemptId: nativeBinding.attemptId, runId: "run:local-hermes", nodeId: nativeBinding.nodeId }, worker: { workerId: binding.workerId,
  ...
  connectorProfileDigest: sha256Digest("profile"), acceptanceProfileId: "profile:local-hermes",
...
test("recovery inspection refuses a stage from a different authenticated delivery", async t => {
  ...
  const other = createControllerWorkerDeliveryV1({ ...otherInput, identity: { tenantId: "tenant:test", projectId: "project:test",
    jobId: "job:other", attemptId: "attempt:other", runId: "run:other", nodeId: "node:local-hermes" }, authorityDigest: sha256Digest("other") });
