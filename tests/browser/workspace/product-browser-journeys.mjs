  ...
  return Object.freeze({
    ...
    journeys: Object.freeze([
      ...
      Object.freeze({ title: "Prepare a revised task and follow its protected follow-up page",
        ...
        proof: "After Request changes the product UI surfaces Prepare revised task because the bootstrap runtime is wired with revisionPlanning: true and the quality scenario from nativeQualityCompletionFixture; clicking the button POSTs once to /revisions, the status reveals an Open revised task link to a fresh task id, the follow-up page renders the Agent progress heading, and a reload does not re-issue the prepare command.",
        simulated: true }),
      ...
      Object.freeze({ title: "Exercise keyboard focus at both 360px and 1280px without sideways scroll",
        command: "(no protected command)",
        proof: "The narrow product page begins with the skip link (a.skip-link, \"Skip to content\"); the wide product page begins with a workspace action link to /tasks. The skip-link branch actually presses Enter and asserts focus reaches the main landmark; the non-skip-link branch reports its layout-specific Enter binding as intentionally not contracted for #214 rather than recording a fake pass. Both reaches are exercised at 360px and 1280px without sideways scroll.",
        simulated: true }),
