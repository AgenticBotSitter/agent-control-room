...
test("every source/configuration field and planned identity is bound by the job input digest", () => {
  ...
  const changed = [
    ...
    (p: typeof work.plan) => { if (p.schema === "control-room.abs-feed-plan/v1") p.configuration.maxBytes++; },
    (p: typeof work.plan) => { if (p.schema === "control-room.abs-feed-plan/v1") p.configuration.maxItems++; },
    (p: typeof work.plan) => { if (p.schema === "control-room.abs-feed-plan/v1") p.configuration.timeoutMs++; },
