...
const localBinding = { localServiceId: "service:local-hermes-hermes", workerId: "worker:local-hermes",
  expectedVersion: "0.21.3", sourceRevision: "00570550" } as const;
const result = { type: "result" as const, session_id: "session:local-hermes", exit_code: 0,
  text: "local Hermes worker completed the controlled task.", tokens: { input: 9, output: 6, total: 15, cache_read: 0, cache_write: 0 },
...
function delivery() {
  return createControllerWorkerDeliveryV1({
    ...
    input: { prompt: input.prompt, instructions: input.instructions }, authorityDigest: sha256Digest("local-hermes-authority"),
    connectorProfileDigest: sha256Digest("local-hermes-profile"), acceptanceProfileId: "profile:local-hermes",
    acceptanceProfileDigest: sha256Digest("local-hermes-acceptance"), issuedAt: at(1000), expiresAt: at(120_000),
  });
}

test("local Hermes worker's local delivery is recorded before one controlled invocation and never auto-runs again after restart", async t => {
...
test("a local policy refusal records no receipt and never invokes local Hermes worker", async t => {
  const f = await nativeTaskFixture(); t.after(f.close);
