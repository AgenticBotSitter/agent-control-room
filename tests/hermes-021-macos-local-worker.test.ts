...
const result = (overrides: Record<string, unknown> = {}) => ({ type: "result", session_id: "session:local-hermes", exit_code: 0,
...
const binding = { localServiceId: "service:local-hermes-hermes", workerId: "worker:local-hermes", expectedVersion: "0.21.3", sourceRevision: "00570550" } as const;
const task = { tenantId: "tenant:local", projectId: "project:local", jobId: "job:local", attemptId: "attempt:local",
  runId: "run:local", nodeId: "node:local-hermes", prompt: "Explain the change.", instructions: "Answer plainly.", deadline: 5000 };

const controllerDelivery = () => createControllerWorkerDeliveryV1({
  identity: { tenantId: "tenant:local", projectId: "project:local", jobId: "job:local", attemptId: "attempt:local", runId: "run:local", nodeId: "node:local-hermes" },
  worker: { workerId: "worker:local-hermes", adapterId: HERMES_021_MACOS_LOCAL_ADAPTER_V1, adapterRevision: "00570550" },
...
test("shared local controller packet maps to one local Hermes worker task and remote delivery cannot invoke the Mac worker", () => {
  const prepared = prepareHermes021MacosTaskV1(controllerDelivery(), { kind: "local", workerId: "worker:local-hermes" }, binding);
  assert.deepEqual(prepared, { ...task, deadline: Date.parse("2026-09-19T12:05:00.000Z") });
  assert.throws(() => prepareHermes021MacosTaskV1(controllerDelivery(), { kind: "remote", workerId: "worker:local-hermes" }, binding),
...
test("Hermes 0.21 local worker accepts exactly one well-formed terminal report", async () => {
  ...
  assert.deepEqual(calls, [{ localServiceId: "service:local-hermes-hermes", task, terminalStage: undefined, signal: undefined }]);
});
...
test("local Hermes worker's normal local runner needs a synchronous policy for the exact shared delivery", async () => {
  let runs = 0, policyCalls = 0;
  const completed = await runAdmittedHermes021MacosLocalTaskV1(controllerDelivery(),
    { kind: "local", workerId: "worker:local-hermes" }, binding, { assertAdmitted(input) {
      policyCalls++; assert.equal(input.delivery.worker.workerId, "worker:local-hermes");
  ...
  await assert.rejects(runAdmittedHermes021MacosLocalTaskV1(controllerDelivery(),
    { kind: "local", workerId: "worker:local-hermes" }, binding, { assertAdmitted() { throw new Error("local_policy_refused"); } },
    { async run() { runs++; return [result()]; } }), /local_policy_refused/);
  assert.equal(runs, 1);

  await assert.rejects(runAdmittedHermes021MacosLocalTaskV1(controllerDelivery(),
    { kind: "local", workerId: "worker:local-hermes" }, binding, { assertAdmitted: (() => Promise.resolve()) as never },
    { async run() { runs++; return [result()]; } }), /hermes_local_worker_unavailable/);
  assert.equal(runs, 1);
});

test("local Hermes worker's local policy accepts only its configured canonical authority", async () => {
  const packet = controllerDelivery();
  const policy = createHermes021MacosLocalTaskPolicyV1({ policyId: "policy:local-hermes-local", binding,
  ...
  let runs = 0;
  const outcome = await runAdmittedHermes021MacosLocalTaskV1(packet, { kind: "local", workerId: "worker:local-hermes" }, binding,
  ...
  await assert.rejects(runAdmittedHermes021MacosLocalTaskV1(foreign, { kind: "local", workerId: "worker:local-hermes" }, binding,
  ...
  await assert.rejects(runAdmittedHermes021MacosLocalTaskV1(sameTextDifferentTask,
    { kind: "local", workerId: "worker:local-hermes" }, binding, port,
  ...
  await assert.rejects(runAdmittedHermes021MacosLocalTaskV1(changedInput, { kind: "local", workerId: "worker:local-hermes" }, binding,
    port, { async run() { runs++; return [result()]; } }), /hermes_021_macos_task_policy_refused/);
  assert.equal(runs, 1);
...
test("local Hermes worker's local connector records what is proven and explicitly refuses unqualified execution", () => {
...
test("local Hermes worker's completed JSON report becomes shared inert result evidence", () => {
  ...
  const evidence = projectHermes021MacosTerminalResultEvidenceV1({
    lineage: { tenantId: "tenant:local", projectId: "project:local", jobId: "job:local", attemptId: "attempt:local",
      runId: "run:local", nodeId: "node:local-hermes" },
    retained: { sessionId: "session:local-hermes", connectorProfileDigest: sha256Digest("profile"),
...
test("the first owner qualification is text-only, bounded, and never retried", async () => {
  const calls: unknown[] = [];
  const qualification = { ...task, qualificationId: "qualification:local-hermes", mode: "text_only" as const,
