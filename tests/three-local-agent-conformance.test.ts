...
import { HERMES_021_MACOS_LOCAL_ADAPTER_V1, runHermes021MacosLocalTaskV1 } from "../src/harness/hermes-021-v1";
import { CLAUDE_CODE_LOCAL_ADAPTER_V1 } from "../src/harness/claude-code-v1";
...
test("C4's two local routes keep distinct pending reviews through lost-reply recovery without an automatic resend", async t => {
  // Both adapters enter the already-tested controller packet/result/review path.
  ...
  assert.notEqual(hermes.topology?.deliveryId, claude.topology?.deliveryId,
    "the shared receiver retains one receipt per local route");
  assert.notEqual(hermes.delivered.artifact.contentHash, claude.delivered.artifact.contentHash,
    "one route's pending-review result cannot be reused as the other route's result");
  assert.equal(hermes.delivered.text, "Hermes pending-review text.");
  assert.equal(claude.delivered.text, "Claude pending-review text.");

  // Reconstructing the receipt receiver models a lost acknowledgement/controller
  // restart. It retains the exact packets and reports duplicate delivery, not a
  // fresh local start or a route substitution.
  const receipts = receiptPort();
  const replay = async (worker: typeof workers[number], lifecycle: typeof hermes) => {
    const packet = deliveryFor(worker);
    const route = { kind: "local" as const, workerId: worker.workerId };
    await deliverControllerWorkerPacketV1(receipts.port, packet, route);
    const restarted = receiptPort(receipts.receipts);
    const duplicate = await deliverControllerWorkerPacketV1(restarted.port, packet, route);
    assert.equal(duplicate.disposition, "duplicate");
    assert.equal(duplicate.startsWork, false);
    assert.equal(lifecycle.topology?.route, "local");
  };
  await replay(workers[0], hermes); await replay(workers[2], claude);
  assert.equal(receipts.receipts.size, 2);

  // An observed cancellation is a terminal no-result outcome. The injected
  ...
  abort.abort();
  assert.deepEqual(await cancelled, { kind: "cancelled", reason: "hermes_local_execution_aborted" });
