import { IdeaLabOperatorServiceErrorV1,IdeaLabProjectLifecycleServiceErrorV1 } from "@/src/idea-lab/v1";
...
export function ideaLifecycleFailureV1(error:unknown){if(!(error instanceof IdeaLabProjectLifecycleServiceErrorV1))return ideaJsonV1({error:"lifecycle_boundary_unavailable"},503);
  if(error.safeCode==="owner_forbidden")return ideaJsonV1({error:error.safeCode},403);
  if(error.safeCode==="project_not_found")return ideaJsonV1({error:error.safeCode},404);
  if(error.safeCode==="state_conflict")return ideaJsonV1({error:error.safeCode},409);
  if(error.safeCode==="invalid_lifecycle_request")return ideaJsonV1({error:error.safeCode},400);
  return ideaJsonV1({error:"lifecycle_boundary_unavailable"},503);}
...
export function exactLifecycleBodyV1(value:unknown):{commandId:unknown;expectedVersion:unknown;requestedAt:unknown}{if(!value||typeof value!=="object"||Array.isArray(value)||Object.getPrototypeOf(value)!==Object.prototype)throw new Error("invalid body");
  const descriptors=Object.getOwnPropertyDescriptors(value),keys=Reflect.ownKeys(value);if(keys.length!==3||keys.some(key=>typeof key!=="string"||!["commandId","expectedVersion","requestedAt"].includes(key)))throw new Error("invalid body");
  for(const key of keys as string[]){const descriptor=descriptors[key];if(!descriptor||!("value" in descriptor)||descriptor.get||descriptor.set||!descriptor.enumerable)throw new Error("invalid body");}
  return{commandId:descriptors.commandId!.value,expectedVersion:descriptors.expectedVersion!.value,requestedAt:descriptors.requestedAt!.value};}
