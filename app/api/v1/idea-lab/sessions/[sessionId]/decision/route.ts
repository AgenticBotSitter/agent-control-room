...
export function createIdeaLabOwnerDecisionHandlerV1(runtime?:IdeaLabProtectedRuntimeV1){return async function handler(request:Request,context:{params:Promise<{sessionId:string}>}){
  ...
  let intent:unknown;try{const length=Number(request.headers.get("content-length")??"0");if(length>4096)throw new Error("too large");intent=await request.json();}catch{return json({error:"invalid_owner_decision"},400);}
