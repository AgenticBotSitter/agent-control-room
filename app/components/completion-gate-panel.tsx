...
export function CompletionGatePanel({ items }: { items: readonly CompletionGateViewModelV1[] }): JSX.Element {
      ...
      {items.map((item) => <li key={item.target.id} className={`completion-gate-item status-${item.status}`}>
        ...
        <div className="completion-gate-grid">
          <section><h4>Review evidence</h4>
            {item.reviews.length === 0 ? <p className="completion-empty">No review evidence recorded.</p> : <ol className="completion-facts">
          ...
          <section><h4>Verification</h4>
            {item.verifications.length === 0 ? <p className="completion-empty">No verification evidence recorded.</p> : <ol className="completion-facts">
        ...
        <div className="completion-gate-grid">
          <section><h4>Findings and lineage</h4>
            {item.findings.length === 0 ? <p className="completion-empty">No open findings are shown for this target.</p> : <ol className="completion-facts">
            ...
            {item.openFindingIds.length > 0 && <p className="completion-lineage">{item.openFindingIds.length} open finding{item.openFindingIds.length === 1 ? "" : "s"} require explicit resolution in a bounded revision.</p>}
        ...
        <section className="completion-previews"><h4>Safe previews</h4>
          {item.previews.length === 0 ? <p className="completion-empty">No digest-addressed preview metadata is available.</p> : <ol>{item.previews.map((preview) => <PreviewSummary key={preview.previewId} preview={preview} />)}</ol>}
        </section>
      </li>)}
    </ol>}
  </section>;
