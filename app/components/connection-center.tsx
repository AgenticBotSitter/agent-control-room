...
export function ConnectionCenterPanel({ data }: { data: ConnectionCenterDataStateV1 }) {
  ...
  return <>
    <section className="metric-grid connection-center-summary" aria-label="Connection summary">
      ...
      <article className="metric-card"><span className="metric-icon blue">⌁</span><div><small>Private SSH</small><strong>{projection.summary.sshConnectionCount}</strong><em>{projection.summary.localConnectionCount} local</em></div></article>
