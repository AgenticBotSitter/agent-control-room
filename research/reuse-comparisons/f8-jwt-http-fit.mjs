...
const root=process.argv[2];assert.match(root,/^\/private\/tmp\/cr-f8-jwt-http\.[A-Za-z0-9]+$/);
...
function modules(kind,{awaitIdentity=true,gate=async()=>{}}={}){
 ...
 function load(name){if(cache[name])return cache[name];let source=sources[name];assert.ok(source);
  if(name==='access-verifier'&&kind!=='current'){
   source=replaceOnce(source,'verify("RSA-SHA256", Buffer.from(`${h}.${c}`), key, signature)',kind==='jose'?'(await candidate(token,key,nowMs,expectedIssuer,audience,maxSessionSeconds),true)':'(candidate(token,key,nowMs,expectedIssuer,audience,maxSessionSeconds),true)');
  ...
  const context=vm.createContext({exports,candidate,Buffer,TextDecoder,URL,Date,Request,Response,setTimeout,clearTimeout,require(spec){if(spec.startsWith('./'))return load(spec.slice(2));assert.ok(['node:crypto','zod'].includes(spec));return local(spec);}});
...
console.log(JSON.stringify({scope:'Actual auth, project-http and http-common modules; mapped JWT call and jose await; synthetic listPage service; no DB, listener, live Access or owner bootstrap.',sourceHashes,versions:{jose:'6.2.12',jsonwebtoken:'9.0.3'},results,count:results.length,maxRssKiB:process.resourceUsage().maxRSS,appWrites:0},null,2));
