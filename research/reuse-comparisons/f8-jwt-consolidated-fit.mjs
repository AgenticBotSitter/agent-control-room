// Research-only consolidated actual-library parsing/verification. No production edits.
...
function mapped(kind){
  let adapted=source.replace('createPublicKey, verify,','createPublicKey,');
  const a=adapted.indexOf('function decode('),b=adapted.indexOf('/** Pure credential',a);
  assert.ok(a>0&&b>a);
  adapted=adapted.slice(0,a)+
    'function validateSegment(value: string, utf8: boolean): void {\n'+
    ...
    '}\n\n'+adapted.slice(b);
  const old='const header = headerSchema.parse(decode(h));';
  assert.equal(adapted.split(old).length,2);
  adapted=adapted.replace(old,'validateSegment(h, true); validateSegment(c, true); validateSegment(s, false);\n      const header = headerSchema.parse(candidateHeader(token));');
  const sigStart=adapted.indexOf('      const signature ='),sigEnd=adapted.indexOf('      const now = nowMs / 1000;',sigStart);
  assert.ok(sigStart>0&&sigEnd>sigStart);
  adapted=adapted.slice(0,sigStart)+'      const claims = claimsSchema.parse('+(kind==='jose'?'await ':'')+'candidate(token, key, nowMs, expectedIssuer, audience, maxSessionSeconds));\n'+adapted.slice(sigEnd);
  ...
  assert.ok(!adapted.includes('JSON.parse(')&&!adapted.includes('verify("RSA-SHA256"'));
  const exports={};
  const candidateHeader=kind==='jose'?token=>jose.decodeProtectedHeader(token):token=>jwt.decode(token,{complete:true})?.header;
  const candidate=kind==='jose'?async(token,key,nowMs,issuer,audience,maxAge)=>(await jose.jwtVerify(token,key,{algorithms:['RS256'],issuer,audience,requiredClaims:['iss','aud','sub','iat','exp'],maxTokenAge:maxAge,currentDate:new Date(nowMs),clockTolerance:0})).payload:(token,key,nowMs,issuer,audience,maxAge)=>jwt.verify(token,key,{algorithms:['RS256'],issuer,audience,maxAge: String(maxAge)+'s',clockTimestamp:nowMs/1000,clockTolerance:0});
  const context=vm.createContext({exports,candidate,candidateHeader,Buffer,TextDecoder,URL,Date,require(name){assert.ok(['node:crypto','zod'].includes(name));return localRequire(name);}});
...
function rawToken(h,c){const body=`${h}.${c}`;return `${body}.${sign('RSA-SHA256',Buffer.from(body),privateKey).toString('base64url')}`;}
const tests=[
 ...
 ['padded header',rawToken(encode(header)+'=',encode(claims)),false],
 ['padded payload',rawToken(encode(header),encode(claims)+'='),false],
 ['invalid UTF8 header',rawToken(Buffer.from([255]).toString('base64url'),encode(claims)),false],
 ['invalid UTF8 payload',rawToken(encode(header),Buffer.from([255]).toString('base64url')),false],
 ['null claims',token(null),false],['empty subject',token({...claims,sub:''}),false],
 ['too many audiences',token({...claims,aud:[trust.audience,...Array(16).fill('other')]}),false],
 ['fractional expiry',token({...claims,exp:seconds+500.5}),false],
 ['unsafe expiry',token({...claims,exp:Number.MAX_SAFE_INTEGER+1}),false],
 ['not before exactly now',token({...claims,nbf:seconds}),true],
];
...
console.log(JSON.stringify({scope:'Actual package header/payload parsing and strongest applicable claim options through hash-pinned current policy; custom JSON.parse and signature call removed in memory. Synthetic keys, no HTTP handler/DB/live Access.',versions:{jose:'6.2.12',jsonwebtoken:'9.0.3'},results,rawWithoutPolicy:raw,timings,parentMaxRssKiB:process.resourceUsage().maxRSS,applicationFilesChanged:0},null,2));
