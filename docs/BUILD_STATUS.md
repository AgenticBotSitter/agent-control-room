...
```text
Active: CR-6Q clean replacement independent review of the remediated fleet/scheduler head; CR-6E owner acceptance remains a separate owner gate
...
Validation completed: CR-6Q's focused suite passes 41 tests with zero failures. The expanded normal full suite now runs 339 tests: 337 pass, zero fail, and two platform-specific tests intentionally skip. Type checking and lint pass. The review closed mixed-node eligibility composition, stale/future fleet evidence presentation, administrative-state-as-online drift, future-granted read scope, future-starting reservations, and missing default regression coverage. Production/rendered build and migration verification remain to be rerun at the final review head. See `CR6Q_ACCEPTANCE.md`.
...
Owner input required now: no message relay is required for the replacement CR-6Q jobber; an eligible non-Ziggy bot route can claim it directly once published. Owner review of the protected dashboard according to `CR6E_ACCEPTANCE.md` remains pending. Exact approval remains required before installing or starting a native service, a new macOS native attempt, credentials, live artifact namespaces, deployment, live integration, or consequential effects
