...
| CR-6Q fleet/scheduler architecture review | Codex review and remediation complete; clean replacement independent review pending | Mixed-node isolation, stale/future evidence, live-status drift, reservation time, and default-suite gaps fixed; first report retained as research only after an install-scope deviation; `CR6Q_ACCEPTANCE.md` |
| CR-7 through CR-10 | Dependency-mapped; implementation not started | `CONTROL_ROOM_COMPLETION_PROGRAM.md` |
...
```text
Active: CR-6Q clean replacement independent review is ready for direct Mac/Linux bot claim in jobber #157; CR-6E owner acceptance remains a separate owner gate
...
Validation completed: CR-6Q's focused suite passes 41 tests with zero failures. The expanded normal full suite runs 339 tests: 337 pass, zero fail, and two platform-specific tests intentionally skip. Type checking and lint pass. Production build and rendered-route tests pass, and migration verification applies 0001 through 0019 and verifies 66 PostgreSQL tables. The review closed mixed-node eligibility composition, stale/future fleet evidence presentation, administrative-state-as-online drift, future-granted read scope, future-starting reservations, and missing default regression coverage. See `CR6Q_ACCEPTANCE.md`.
...
Owner input required now: no message relay is required for replacement jobber #157; the Mac or Linux bot route can claim it directly. Owner review of the protected dashboard according to `CR6E_ACCEPTANCE.md` remains pending. Exact approval remains required before installing or starting a native service, a new macOS native attempt, credentials, live artifact namespaces, deployment, live integration, or consequential effects
...
```text
Block: CR-6Q — independent fleet/scheduler security review and Codex disposition
Set model: gpt-5.6-sol
Set reasoning effort: xhigh
Why: This phase checks cross-tenant evidence composition, stale platform claims, scheduler bypass, starvation, capacity races, and redaction before harness adapters can consume these contracts.
Expected output: an independent report against the immutable remediated head, followed by Codex closure or remediation of every finding and final build/render/migration evidence.
Stop before: making any live operation, starting a native service, dispatching an external job, changing credentials or production identities, deployment, or consequential external effects without exact owner authority.
```
