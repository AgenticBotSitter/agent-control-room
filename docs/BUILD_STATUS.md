...
| CR-5D | In progress — execution, delivery, lineage, safe node controls, UI integration, and recovery acceptance complete; disposable storage next | Synthetic executor, evidence builder, coordinator, storage boundary, restart-safe SQLite event/outbox/lineage/control delivery, integrated worker controls and evidence, abrupt-exit recovery, and deterministic tests |
