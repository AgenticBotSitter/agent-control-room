...
| CR-7B Codex worker adapter | Effect-free Mac adapter complete; native qualification blocked by accepted credential-isolation failure | Signed-binary manifest, authority-bound command planner, safe decoder/process/worktree/artifact lineage, plus one sanitized native negative fixture proving read-only commands could reach the disposable saved-auth file; `CR7B_ACCEPTANCE.md` |
...
```text
Active: CR-7B credential-isolated native execution design and requalification; CR-6E owner acceptance remains a separate owner gate
...
CR-7B effect-free and negative-evidence validation now passes 14/14 focused tests. The combined full suite runs 365 tests: 363 pass, zero fail, and two platform-specific tests intentionally skip. Type checking, focused lint, production build, two rendered-route tests, and migration verification through 0020/68 tables pass. Safe patch publication, identity-bound worktree cleanup, sanitized native negative evidence, and a mandatory short-lived broker-permit boundary are implemented. The first authorized provider call proved the disposable saved-auth file was readable to a model-controlled command; the harness stopped and removed both temporary targets. See `CR7B_ACCEPTANCE.md`.
Open risks: native Codex saved authentication is readable across the tested read-only command boundary, so native execution remains disabled until long-lived credentials are moved outside the worker and a bounded broker/proxy or equivalent OS boundary passes review. Approval response is unqualified; Hermes empty/invalid toolset configuration fails open to configured tools, so the exact valid zero-tool selection and observed count are mandatory; raw Hermes config display may expose credential-bearing MCP URLs and is forbidden from adapter capture. The macOS provider remains unqualified and its original failure stage remains unproven; atomic platform file semantics, live DNS/TLS and rebinding, timer/cancellation transport, live cost/resource consumption, process-kill rehearsal, destination evidence, approval issuance, disposable deployment, and CR-6 service isolation remain explicit downstream gates
Owner input required now: none for effect-free credential-boundary design and review. A further native provider call is forbidden until that design exists and then requires new exact disposable-scope approval. Owner review of the protected dashboard according to `CR6E_ACCEPTANCE.md` remains pending. Exact approval remains required before installing or starting a native service, enabling effect-capable Hermes tools or MCP servers, a new macOS native attempt, new credentials, live artifact namespaces, deployment, live integration, or consequential effects
...
```text
Block: CR-7B — credential-isolated native Codex execution
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: The first authorized native call proved that the read-only sandbox still allowed a model-controlled command to see the disposable saved-auth file. The adapter must now keep long-lived credentials outside the worker command boundary before any lifecycle qualification can pass.
Expected output: reviewed credential-broker/proxy or equivalent OS-enforced contract, adversarial unreadability tests, then separately authorized sanitized start/event/usage/cancel/explicit-ID-resume evidence.
