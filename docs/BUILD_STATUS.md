...
| CR-7B Codex worker adapter | Effect-free Mac adapter, worktree safety, result/file/test lineage, and patch artifact publication complete; native qualification pending | Signed-binary manifest, authority-bound command planner, stdin prompt isolation, JSONL decoder, cancellation/deadline process contract, identity-bound cleanup, exact artifact lineage, and focused conformance tests; `CR7B_ACCEPTANCE.md` |
...
```text
Active: CR-7B native read-only Codex qualification and conformance completion; CR-6E owner acceptance remains a separate owner gate
...
```text
Block: CR-7B — native Codex qualification and result lineage
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: The signed-binary manifest, authority mapping, safe command plan, JSONL decoder, cancellation contract, worktree identity/cleanup boundary, content-free results, and verified patch artifact publication now exist. The remaining work is proving read-only credential isolation against the exact native CLI and completing native conformance.
Expected output: sanitized native read-only start/event/usage/cancel/resume evidence, credential-isolation proof, and final adversarial native conformance evidence.
Stop before: any child Codex process without exact disposable approval; workspace-write until read-only credential isolation passes; production worktree mutation, credential disclosure, deployment, or consequential external effect.
```
