...
| CR-6D services, schedules, incidents, and reconciliation | In progress — recurrence, DST handling, desired/observed reconciliation, durable incident correlation, occurrence/outbox persistence, and delivered-outbox restart reconciliation implemented | `CR6D_SERVICE_SCHEDULE_CONTRACT.md`, migrations 0016/0017, registered unit and persistence tests |
...
Validation completed: CR-6C passed type checking, lint, 287 full-suite tests with zero failures and two intentional skips, the production/rendered build, migrations 0001 through 0015, seeded scheduling properties, the named failure-mode catalogue, restart reconciliation, and 16-way capacity contention. CR-6D now has deterministic cron/interval/once calculation, explicit IANA timezone/DST semantics, desired-versus-observed service reconciliation, durable incident correlation, and a transactionally materialized occurrence/outbox ledger with exact replay, cross-tenant collision coverage, and delivered-outbox/pending-occurrence restart recovery. Full verification currently passes 299 tests with zero failures and two intentional skips, the production/rendered build, and migrations 0001 through 0017. See `CR6C_ACCEPTANCE.md` and `CR6D_SERVICE_SCHEDULE_CONTRACT.md`.
