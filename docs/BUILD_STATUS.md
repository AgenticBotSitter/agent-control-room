...
| CR-6C deterministic scheduling and reservations | In progress — allocation, starvation, placement, hard constraints, resource and budget reservations, and bottleneck reporting implemented | Versioned scheduler contract, migrations 0014/0015, production modules, and registered deterministic/property tests |
| CR-6D through CR-10 | Dependency-mapped; implementation not started | `CONTROL_ROOM_COMPLETION_PROGRAM.md` |
...
```text
Active: CR-6C deterministic scheduler, resource reservations, fairness, and bottlenecks
...
Open risks: the macOS provider remains unqualified and its original failure stage remains unproven; atomic platform file semantics, live DNS/TLS and rebinding, timer/cancellation transport, approval consumption, live cost/resource consumption, reservation reconciliation, process-kill rehearsal, destination evidence, approval issuance, disposable deployment, and CR-6 service isolation remain explicit downstream gates
Owner input required now: none for the effect-free CR-6A contract or static package implementation; exact approval remains required before installing or starting a native service, a new macOS native attempt, credentials, live artifact namespaces, deployment, live integration, or consequential effects
...
```text
Block: CR-6C — deterministic scheduler, resource reservations, fairness, and bottlenecks
Set model: gpt-5.6-sol
Set reasoning effort: xhigh
Why: This phase establishes the allocation and starvation boundaries that later services and operator controls must obey.
Expected output: versioned deterministic allocation/explanation contract, fair-share debt and priority/deadline/cost/privacy/maintenance rules, atomic resource reservations and recovery, bottleneck calculation, simulation/property tests, and safe explanations.
Stop before: dispatching a real job, reserving a live GPU or host resource, changing a service, credentials, production identities, live deployment, or consequential external effects without exact owner authority.
```
