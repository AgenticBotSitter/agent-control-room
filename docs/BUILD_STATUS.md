...
| CR-7 foundation and Hermes adapter | Foundation and effect-free discovery/read slice complete; disposable provider-keyed lifecycle pending owner authority | Strict harness contracts, migration 0020, replay-safe run/event persistence, Session Watch read source, pinned Hermes manifest/compatibility gate, sanitized gateway fixtures, and read-only serve projection; `CR7A_ACCEPTANCE.md` |
| CR-7B through CR-10 | Dependency-mapped; not started | `CONTROL_ROOM_COMPLETION_PROGRAM.md` |

## Active block

The CR-7 harness foundation and effect-free Hermes discovery/read slice are complete. Harness runs now have strict manifest, lifecycle, event, usage, lineage, replay, tenant, project, attempt, and node boundaries. Hermes is pinned to package `0.20.6` at revision `4956ff0cb9646aaf894c228e6dc932b126d1f927`; drift fails closed. The next work is the separately authorized disposable provider-keyed lifecycle. CR-6E owner acceptance, real per-platform supervisor rehearsals, and the unresolved macOS CR-5C.9H native gate remain separate owner-controlled gates.

```text
Active: CR-7A disposable Hermes lifecycle capture and gateway-client acceptance; CR-6E owner acceptance remains a separate owner gate
...
Validation completed: CR-7A's focused effect-free suite passes 8 tests with zero failures. The normal full suite runs 348 tests: 346 pass, zero fail, and two platform-specific tests intentionally skip. Type checking and lint pass. Production build and both rendered-route tests pass, and migration verification applies 0001 through 0020 and verifies 68 PostgreSQL tables. See `CR7A_ACCEPTANCE.md`.
...
Owner input required now: exact approval for one bounded, disposable, provider-keyed Hermes lifecycle is required before live adapter discovery or implementation of start/steer/cancel/resume support. No install or service start is implied. Owner review of the protected dashboard according to `CR6E_ACCEPTANCE.md` also remains pending. Exact approval remains required before installing or starting a native service, a new macOS native attempt, credentials, live artifact namespaces, deployment, live integration, or consequential effects
...
```text
Block: CR-7A — disposable Hermes lifecycle and pinned gateway client
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: The shared contract and read-only normalization are complete; the remaining version-sensitive lifecycle verbs must be proven against one exact disposable Hermes turn before code may claim support.
Expected output: sanitized start/event/usage/wait/cancel/reconnect/resume fixtures at the exact pin, followed by an authority-bound gateway client and conformance tests.
Stop before: any provider-keyed turn until the owner authorizes its exact disposable scope; any production profile, repository mutation, unrestricted prompt, credential disclosure, service start, deployment, or consequential external effect.
```
