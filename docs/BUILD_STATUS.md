...
| CR-7 through CR-10 | Dependency-mapped; CR-7A is next | `CONTROL_ROOM_COMPLETION_PROGRAM.md` |

## Active block

CR-6Q is complete: the fleet, scheduler, service, and operator-surface boundary passed Codex review and a clean independent replacement review, and both substantive independent findings are remediated. CR-7A is now the active effect-free block. CR-6E owner acceptance, real per-platform supervisor rehearsals, and the unresolved macOS CR-5C.9H native gate remain separate owner-controlled gates; none is converted into a pass by advancing the adapter foundation.

```text
Active: CR-7A Hermes adapter foundation and lifecycle discovery; CR-6E owner acceptance remains a separate owner gate
...
Validation completed: CR-6Q's focused suite passes 42 tests with zero failures. The expanded normal full suite runs 340 tests: 338 pass, zero fail, and two platform-specific tests intentionally skip. Type checking and lint pass. Production build and both rendered-route tests pass, and migration verification applies 0001 through 0019 and verifies 66 PostgreSQL tables. The review closed mixed-node eligibility composition, stale/future fleet evidence presentation, administrative-state-as-online drift, future-granted read scope, future-starting reservations, maximum signal lifetime enforcement, terminated-reservation replay, and missing default regression coverage. Replacement jobber #157 and PR #158 were accepted as clean independent evidence. See `CR6Q_ACCEPTANCE.md`.
...
Owner input required now: none for effect-free CR-7A implementation. Owner review of the protected dashboard according to `CR6E_ACCEPTANCE.md` remains pending. Exact approval remains required before installing or starting a native service, a new macOS native attempt, credentials, live artifact namespaces, deployment, live integration, or consequential effects
...
```text
Block: CR-7A — Hermes adapter foundation and lifecycle discovery
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: This phase introduces the first harness adapter boundary and must map Hermes lifecycle facts into Control Room contracts without granting the adapter scheduling, approval, or effect authority.
Expected output: a versioned Hermes adapter contract, effect-free lifecycle discovery and normalization, bounded fixtures, and tests proving identity, tenant, state, replay, and redaction behavior.
Stop before: connecting to a live Hermes deployment, reading credentials or private payloads, dispatching external work, starting a service, deployment, or consequential external effects without exact owner authority.
```
