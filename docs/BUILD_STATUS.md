...
```text
Active: CR-7B Codex worker adapter, structured lifecycle decoder, and sandbox/worktree authority mapping; CR-6E owner acceptance remains a separate owner gate
...
```text
Block: CR-7B — Codex worker adapter and normalized lifecycle
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: CR-7A proved the Hermes lifecycle. CR-7B now adds the second real harness using Codex's structured process protocol, with explicit sandbox, directory, worktree, cleanup, and attempt-authority mapping.
Expected output: sanitized `codex exec --json` fixtures, a bounded process wrapper and decoder, normalized start/stream/steer/cancel/resume/result/usage lineage, and adversarial conformance tests.
Stop before: a live unrestricted Codex run, production worktree mutation, credential disclosure, deployment, or consequential external effect.
```
