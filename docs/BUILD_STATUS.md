...
| CR-7B Codex worker adapter | Effect-free Mac adapter and durable credential-broker policy core complete; native qualification blocked pending real OS-isolated broker review | Signed-binary manifest, authority-bound command planner, safe decoder/process/worktree/artifact lineage, accepted native negative evidence, at-most-once broker policy, and private durable restart ledger; `CR7B_ACCEPTANCE.md` |
...
```text
Active: CR-7B OS-isolated credential-broker process boundary and requalification; CR-6E owner acceptance remains a separate owner gate
...
CR-7B effect-free, negative-evidence, and credential-broker validation now passes 21/21 focused tests. The combined full suite runs 372 tests: 370 pass, zero fail, and two platform-specific tests intentionally skip. Type checking, focused lint, production build, two rendered-route tests, and migration verification through 0020/68 tables pass. Safe patch publication, identity-bound worktree cleanup, sanitized native negative evidence, short-lived broker permits, atomic provider-call spending, conflict/expiry/limit enforcement, cancellation closure, private durable storage, prompt-canary exclusion, and restart-to-ambiguity recovery are implemented. ADR-049 and `CR7B_CREDENTIAL_BROKER_CONTRACT.md` freeze the security boundary. The first authorized provider call remains the only native Codex call; its temporary targets were removed. See `CR7B_ACCEPTANCE.md`.
...
```text
Block: CR-7B — OS-isolated native Codex credential broker
Set model: gpt-5.6-sol
Set reasoning effort: high
Why: The durable at-most-once broker contract is implemented, but native execution cannot pass until the real process and network boundary proves that the worker cannot reach credentials, internal broker authority, or the provider directly.
Expected output: broker process/IPC/launcher contract with separate OS identity, private credential and ledger access, authenticated run-scoped requests, broker-only provider egress, adversarial boundary tests, then separately authorized sanitized start/event/usage/cancel/explicit-ID-resume evidence.
