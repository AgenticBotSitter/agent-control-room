...
Project and worker detail pages load the same authenticated, tenant-bound snapshot as the dashboard and show a small protected status panel only when its stable identifier matches a returned record. All other detail material remains explicitly labelled synthetic fixture content. These pages provide keyboard skip links and never claim that fixture status, history, assignments, capacity, or capabilities are protected observations.
