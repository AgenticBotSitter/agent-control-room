...
Writes use a create-exclusive lock, a private pending file, file synchronization, create-once linking, directory synchronization, and exact readback. Only a lock acquired by the current operation is removed, and only after bounded successful cleanup. Once the lock has been acquired, any filesystem-operation failure, cancellation, or timeout makes the adapter terminally uncertain: it returns no descriptor and refuses every subsequent call. Before returning that refusal, it records a fixed private uncertainty marker create-once under the captured root. That marker closes the late-cleanup case where the pending file and lock were removed but their final retirement could not be proved durable. A new adapter treats the marker, as well as any retained lock or pending file, as unresolved evidence and refuses the root. Clean success creates no marker and preserves normal exact replay after restart. A stale lock, pending file, uncertainty marker, foreign entry, symlink, hardlink, changed root, changed file identity, mutation, truncation, or malformed envelope also fails closed as `storage_ambiguous`.

The adapter does not repair or delete uncertain files, including the fixed marker. Operators must preserve the root and reconcile uncertain storage outside this adapter. Restarting the adapter is not permission to recover a lock or retry a write.

The tests use actual filesystem operations in disposable private directories. A separate test-only factory gates those same operations to inject failures, cancellation, and timeout at write, sync, create-once-link, and cleanup boundaries; production construction has no injection argument. Tests cover restart replay, concurrent conflict, quotas, cancellation/deadline refusal, terminal uncertainty and subsequent-call refusal, durable restart refusal after an ambiguous lock unlink or final directory sync, mutation and truncation, traversal forms, symlink and hardlink custody, root substitution, and preservation of stale or foreign evidence. These tests are adapter evidence only; they do not qualify a production filesystem, backup process, or complete result-publication path.
