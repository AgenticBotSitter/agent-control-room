...
- Signing boundary: `createBoundedOwnerSignature` with scripted
  ...
  signature over the fixed proof bytes — neither is key material. The valid
  channel returns the minted signature (the production signer verifies it
  internally before resolving); the revoked channel refuses (revoked
  consent). A malformed identity still refuses at construction. The valid
  staged-completion journey signs the proof material through the valid
  channel.
...
|---|---|
| `valid-staged-completion` | `pass` (synthetic): staged rev1→rev2, flushed, observed, restore bundle matches, proof material signed (64-byte signature) |
...
Every record carries `workApproved: false`. The three `pass` records carry
