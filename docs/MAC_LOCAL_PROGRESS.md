...
- REAL 2026-09-25 — owner guide updated with an explicit known-limitations section. The guide's banner previously said nothing had been executed against the real database, which is now wrong: the four-role check and the wrong-server-name refusal are REAL passes. The banner now separates the proven database route from the stack, which has still never been started. Section 9 states plainly that the system cannot start yet, that cancel does not exist at all, that assign/approve/accept/request-changes/plan are routed but unusable, that no service runs at login, and that checks 7-9 still need a person. Section 8 was stale and told the owner the Tailscale tag was an open action; it now records both the tag and the policy grant as done and points at the remaining drills.

- REAL 2026-09-25 — M2 browser journey tests landed: 14 cases (7 reachable journey steps, 5 routed-but-unusable refusals, the absent cancel route, and preview/demo path refusal), passing against a disposable local PostgreSQL 17 cluster in the integration checkout. The registered `test:mac-local-browser-journeys` script was re-run in the checkout that owns a real `node_modules` and passed 14/14 there. Correction to an earlier note in this session: a pnpm run reported as `EXIT=0` had actually failed, because the exit code was read after a pipe and therefore described `tail` rather than the test; the suite summary line is the authoritative signal, and the passing result above comes from that summary, not from a shell status. Note for future lanes: `pnpm <script>` cannot run in a worktree whose `node_modules` is a symlink, because pnpm's dependency-status check aborts before the script runs; use the underlying `node --import tsx --test` there, and use the pnpm wrapper only in the real checkout. No leftover disposable database remained after the runs.

- REAL 2026-09-25 — Phase 1 resumed after the owner applied the single tailnet rule (`tag:control-room-client` to `tag:control-room-vps`, database port only). Check 4 now passes: `mac:check-database` returned `ok` for all four roles (web, coordinator, results, queueWorker), exit 0, against the real VPS database over the direct route with no SSH and no tunnel. Check 5 passes: on a throwaway copy of the protected configuration with a deliberately wrong `serverName`, all four roles refused with exit 1, the wrong name was never echoed, and no password appeared in output; an unmodified copy of the same files still passes, so the refusal is caused by the wrong name and not by the copy. The protected original was never opened for write and was re-checked healthy afterwards. Check 3 is recorded as owner-attested, not machine-verified: one new rule, source `tag:control-room-client`, destination `tag:control-room-vps`, port `tcp:5432`, existing rules unchanged; the tailnet policy file is not readable from the Mac, so the diff itself is not machine-verified, while the effect (the port connects and check 4 passes) is REAL. Check 10: the VPS peer's SSH port is reachable and the database re-check passes before and after; no SSH session was opened and no remote command was run. Check 6 was previously accepted on the VPS side and was not re-run. Checks 7, 8 and 9 are owner-pending: check 7 needs the phone or PC port test, check 8 needs a Mac sleep/wake and VPS-side restarts, and check 9 needs a forced certificate renewal by the VPS operator. No SSH fallback was restored at any point, and no password was moved, printed or changed.

- REAL 2026-09-25 — Phase 1 stopped at check 4, as the plan's stop rules require. The `--repoint-only` provisioner printed only `{"repointed":true}`; no address, name or password was printed. `mac:check-database` then returned `database_check_refused` for all four roles. Read-only diagnosis: the Mac shows both approved client tags and tailnet pings the database host peer in about 40ms, and the protected role files all carry the new address, port and v2 endpoint policy with their passwords preserved, but a TCP connection to the database port on that peer times out while the same peer's SSH and HTTPS ports connect at once. No credential was ever sent, so this is a tailnet policy drop, not a password mismatch, and no SSH fallback was restored or attempted. Added as blocking owner action 2 in `OWNER_ACTIONS.md`. Separately, M1's inventory found that `mac:up` cannot start at all: it requires a default task provider build artifact that no `vite.vps.config.ts` entry produces and that appears in no branch history, so Phase 2 is blocked by missing source, independently of the route. Check 5 and checks 7-10 were not attempted.

- REAL 2026-09-25 — Phase 0 landed. The 11 reviewed repoint files were committed by name on `claude/mac-local-support` (403 changed lines, under the 800-line limit) and the finish plan was added as `docs/MAC_LOCAL_FINISH_PLAN.md`, then pushed. `origin/claude/mac-local-support` was merged into `claude/mac-local-integration`; both sides' progress entries are kept and the pre-revision-3.1 "no Tailscale administrator change is required" note is marked superseded rather than deleted. On the merged result the focused repoint test passes 5/5 and `pnpm check` is clean. The `claude-review` gate on the support diff returned `VERDICT: APPROVE` (exit 0), with two non-blocking notes: the two protected config files are renamed separately rather than as one transaction, and the route digest is shape-checked but not recomputed at repoint time. No live step has run yet and no protected root was read.
...
- 2026-09-24 — REAL W1 review gate blocked: the configured Claude review invocation was refused before source transmission because this real-database provisioning diff needs explicit owner approval for that external review; a separate Hermes/Marvin read-only review attempt returned no verdict, so neither is treated as approval and no database rerun occurred.
- 2026-09-24 — REAL rehearsal refresh: the disposable local PostgreSQL rehearsal passed all four restricted-role checks and created its first local owner. After refreshing the installed worker pins, real text-only adapter probes passed for Codex, Claude, and Hermes: each completed one tiny task, cancellation and deadline handling both reaped the full process group, and no task host or production service was started. The remaining W6 blocker is still the real task-provider composition, not agent availability.
- 2026-09-24 — REAL W1 exposure check: before changing anything, the VPS database listener was confirmed loopback-only and its Tailscale Serve configuration contained only the existing private website route. No direct tailnet PostgreSQL forward existed to remove; the existing website and Tailscale SSH remained available. No database, tunnel, firewall, ACL, or service was changed.
- 2026-09-24 — REAL W1/W6 preflight refresh: `pnpm mac:repin` confirmed the installed Codex, Claude, and Hermes pins. With no loopback database-tunnel listener running, the four-role `pnpm mac:check-database` check refused every role rather than using a direct network fallback. No host, worker, tunnel, or database was started or changed.
- 2026-09-24 — REAL W1 secure-route check #7: a direct unauthenticated PostgreSQL protocol probe from this Mac was refused after the Tailnet policy change, and the existing private website returned its normal sign-in redirect. The Mac's tagged-device SSH rule needs an owner choice before the SSH tunnel can be revalidated; no local SSH database tunnel was running during this check.
