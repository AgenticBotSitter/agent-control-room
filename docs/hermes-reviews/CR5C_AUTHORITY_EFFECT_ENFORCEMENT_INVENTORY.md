...
| A5 | Cost ceiling (`maxCostUsd`) | defined `types.ts:56` | — none | **No consumer found.** Not read by claim, renewal, or effect authorization | grep across `src/` finds no reader of `maxCostUsd` | Entire cost budgeting is unenforced end-to-end | **ambiguous** → likely deferred to CR-6C budgets; must be stated explicitly in CR-5C docs |
...
| C3 | Executor restriction (`allowedExecutor`) | field exists `types.ts:49` | **no consumer found** — no code compares attempt worker/node capability to `allowedExecutor` | grep over `src/persistence`, `src/node-*` shows zero reads | A job can be claimed by any active node regardless of declared executor | **decision required**: enforce centrally at offer time, node-locally at accept time, or both |
