...
CR11A-TEAM-060 is accepted for a repository-only filtered-read candidate. The future native method is fixed to one profile, optional one room, digest selectors, bounded metadata, and a sixty-second Ed25519 profile/device attestation. The Control Room bridge accepts only injected signed fixtures, retains safe metadata and digests, has an empty accepted-runtime list, is disabled by default, and exposes no native reader or effect method. See `CR11A_TEAM_060_FILTERED_HERMES_READ_CONTRACT.md`, `CR11A_TEAM_060_HERMES_UPSTREAM_IMPLEMENTATION_NOTE.md`, and `CR11A_TEAM_060_ACCEPTANCE.md`.
