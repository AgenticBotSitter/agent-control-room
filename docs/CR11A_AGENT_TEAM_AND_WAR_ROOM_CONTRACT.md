...
A later read-only Hermes Bot Mode adapter must separately pin an exact upstream version, normalize only safe projections, prove device and profile identity, reject shared provider access, bind activity to authenticated evidence, preserve Control Room room ceilings, and remain incapable of dispatch or authority mutation.
