# CR13A-LIVE-240 independent remediation third rereview packet
...
The reviewer must begin with a clean procedural slate. Before the fixed sequence, read the architecture, original
packet and rejection, both rereview packets and their procedural rejections, remediation source, exact product diff,
tests, package scripts, barrel export, and directly imported helper. That pre-inspection must not execute, copy into a
shell, or otherwise consume any fixed command listed below. Run the fixed commands only in the fresh disposable clone,
exactly once, in their listed order.

Do not install, download, modify, repair, create executable review code, import or consume LIVE-220 or LIVE-190, invoke
or patch native code, or contact an external system. Stop, clean up, and reject on failure or uncertainty.
...
Use a fresh local-only detached clone with copied prepared dependencies. Run exactly once and in order:
...
suite, generated program, retry, substitution, fallback, repair, or native effect is permitted.
