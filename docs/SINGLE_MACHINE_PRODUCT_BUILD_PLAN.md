...
| --- | --- | --- |
| Hermes Agent | Local queue route, task plan, controlled subprocess shape, terminal staging, result publication, review handoff and restart-safe receipt behavior. | Owner-selected private runner binding; exact runner-bridge qualification; protected data/restart/backup proof; a restricted task policy for real project work. The current `bot_room` text runner is **not** general code-writing support. |
| Claude Code | Bounded stream-json decoding, owned injected session cleanup and canonical result publication. | Installed-process acquisition, queue/admission wiring, approved permission policy, authenticated qualification, cancellation/recovery and exact session behavior. Current profile correctly says every live operation is unsupported. |
