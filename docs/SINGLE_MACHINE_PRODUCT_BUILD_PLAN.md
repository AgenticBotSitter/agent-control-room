...
| Claude Code | Bounded stream-json decoding, owned injected session cleanup and canonical result publication, plus an opaque plan-bound readiness record for installed-process identity, permission limits, and cancellation/restart behavior. | Installed-process acquisition, queue/admission wiring, authenticated owner proof, and exact session behavior. Current profile correctly says every live operation is unsupported until those requirements and a separate enablement decision are complete. |
