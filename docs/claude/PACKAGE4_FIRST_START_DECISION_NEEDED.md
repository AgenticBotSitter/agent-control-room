...
The unspendable key also needs an independent public-key pin. On restart, the current source checks that each database public key and its database fingerprint agree, but a database-side replacement of **both** values with an attacker-controlled key would still pass and permit a signed remote frame. The original public key is random, so it cannot be re-derived from configuration. Section 8.B's “differing existing row is refused” cannot be proven for key material without keeping the generated public fingerprint independently (for example in a protected Mac file). That would contain no private key or secret, but the exact custody/rotation rule needs Claude's security decision. The current source has not been run live and must not be approved as the final key bootstrap on this evidence.
