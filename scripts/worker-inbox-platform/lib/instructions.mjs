...
export function instructionsFor({ platform, workerId, artifactDirectory, runtimeDirectory, repository, signalDirectory }) {
  ...
  if (platform === "windows") {
    ...
    return [
      ...
      `  schtasks /Query /TN "${task}" /V /FO LIST | findstr /C:"Last Run Time" /C:"Last Result" /C:"Next Run Time" /C:"Status"`,
      `  type "${runtimeDirectory}\\watch.log"`,
      "",
      "If the importer rejects the XML (a contributor had to convert it before",
      "Task Scheduler accepted it): the emitted bytes are UTF-8 and match the",
      "declaration, so convert a COPY to UTF-16 and import that instead —",
