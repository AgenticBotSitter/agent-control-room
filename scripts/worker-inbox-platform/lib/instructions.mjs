...
export function instructionsFor({ platform, workerId, artifactDirectory, runtimeDirectory, repository, signalDirectory }) {
  ...
  if (platform === "launchd") {
    ...
    return [
      ...
      "",
      "Inspect (did it run, and what happened):",
      `  launchctl print gui/$(id -u)/${label} | grep -Ei "state|last exit code|last run"`,
  ...
  if (platform === "systemd") {
    ...
    return [
      ...
      "",
      "Inspect (last/next run plus the last result):",
      `  systemctl --user list-timers ${base}.timer`,
      `  systemctl --user status ${base}.service -n 0 --no-pager`,
  ...
  if (platform === "windows") {
    ...
    return [
      ...
      "",
      "Inspect (last run time plus the last result code):",
      `  schtasks /Query /TN "${task}" /V /FO LIST`,
      `  powershell -NoProfile -Command "Get-ScheduledTaskInfo -TaskName '${task}' | Select-Object TaskName, LastRunTime, LastTaskResult, NextRunTime, NumberOfMissedRuns"`,
      `  type "${runtimeDirectory}\\watch.log"`,
      "",
      "The full schtasks list needs no English-only text filter; the",
      "PowerShell line returns objects, so it works regardless of the",
      "display language. LastTaskResult is the exit code of the last",
      "tick (0 clean, 2 read failure).",
      "",
      "If the importer rejects the XML (a contributor had to convert it before",
      "Task Scheduler accepted it): the fixture confirms the artifact is",
      "internally UTF-8-consistent, but that does not diagnose the rejection.",
      "Do not change the generator without new evidence. As a workaround,",
      "convert a COPY to UTF-16 and import that instead — name the input",
      "encoding explicitly, because the file is deliberately BOM-less and",
      "Windows PowerShell 5.1 would otherwise decode non-ASCII paths with",
      ...
      "The UTF-8 artifact stays the owned original; the converted copy is an",
      "importer-side workaround, verified only by the operator's own import.",
      "",
      "Stop (keep files, keep ownership):",
