...
const sourcePath = fileURLToPath(new URL("../native/claude-code-process-v1.c", import.meta.url));
const sha256 = bytes => createHash("sha256").update(bytes).digest("hex");
const refused = () => { throw new Error("claude_code_process_native_build_refused"); };
export const CLAUDE_CODE_PROCESS_NATIVE_CFLAGS_V1 = CLAUDE_CODE_PROCESS_NATIVE_REVIEWED_CFLAGS_V1;

export async function buildClaudeCodeProcessNativeArtifactV1({ outputDirectory }) {
  ...
  try {
    const name = "claude-code-process-v1", executable = join(work, name);
    const source = await readFile(sourcePath);
    const capturedSource = join(work, "claude-code-process-v1.c");
    await writeFile(capturedSource, source, { flag: "wx", mode: 0o600 });
    const args = [...CLAUDE_CODE_PROCESS_NATIVE_CFLAGS_V1, "-isysroot", sdk,
    ...
    const manifest = {
      schema: "control-room.claude-code-process-native-artifact/v1", platform: "darwin", architecture: process.arch,
      minimumMacos: "13.0", protocol: "ACRCCP1", sourceSha256: sha256(source),
      toolchain: { compiler, sdkVersion, flags: [...CLAUDE_CODE_PROCESS_NATIVE_CFLAGS_V1] },
      files,
    ...
    const archiveRoot = `agent-control-room-claude-code-process-darwin-${process.arch}`;
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    console.log(JSON.stringify(await buildClaudeCodeProcessNativeArtifactV1({ outputDirectory: process.argv[3] })));
  } catch {
    console.error("Control Room claude-code-process native build refused.");
    process.exitCode = 1;
  }
}
