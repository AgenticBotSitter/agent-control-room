...
const sourcePath = fileURLToPath(new URL("../native/installed-configuration-v1.c", import.meta.url));
const sha256 = bytes => createHash("sha256").update(bytes).digest("hex");
const refused = () => { throw new Error("installed_configuration_native_build_refused"); };
export const INSTALLED_CONFIGURATION_NATIVE_CFLAGS_V1 = INSTALLED_CONFIGURATION_NATIVE_REVIEWED_CFLAGS_V1;

export async function buildInstalledConfigurationNativeArtifactV1({ outputDirectory }) {
  ...
  try {
    const name = "installed-configuration-v1", executable = join(work, name);
    const source = await readFile(sourcePath);
    const capturedSource = join(work, "installed-configuration-v1.c");
    await writeFile(capturedSource, source, { flag: "wx", mode: 0o600 });
    const args = [...INSTALLED_CONFIGURATION_NATIVE_CFLAGS_V1, "-isysroot", sdk,
    ...
    const manifest = {
      schema: "control-room.installed-configuration-native-artifact/v1", platform: "darwin", architecture: process.arch,
      minimumMacos: "13.0", protocol: "ACRCFG1", sourceSha256: sha256(source),
      toolchain: { compiler, sdkVersion, flags: [...INSTALLED_CONFIGURATION_NATIVE_CFLAGS_V1] },
      files,
      ownerQualified: false,
    };
    const manifestName = "INSTALLED_CONFIGURATION_MANIFEST.json";
    const manifestBytes = Buffer.from(`${JSON.stringify(manifest, null, 2)}\n`);
    await writeFile(join(work, manifestName), manifestBytes, { flag: "wx", mode: 0o644 });
    const archiveRoot = `agent-control-room-installed-configuration-darwin-${process.arch}`;
    ...
    await chmod(join(outputDirectory, archiveName), 0o644);
    await chmod(join(outputDirectory, manifestName), 0o644);
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    console.log(JSON.stringify(await buildInstalledConfigurationNativeArtifactV1({ outputDirectory: process.argv[3] })));
  } catch {
    console.error("Control Room installed-configuration native build refused.");
    process.exitCode = 1;
  }
}
