// Workspace fixture smoke check. Starts vite on the workspace preview
// config, asserts the loopback URL serves the entry, then proves the
// port is freed and no tracked file changed. Usage:
//   node scripts/workspace-preview-smoke.mjs
...
const child = spawn(process.execPath, ["node_modules/vite/bin/vite.js", "--config", "vite.workspace-preview.config.ts"],
...
try {
  ...
  if (!page.body.includes("<title>") || !page.body.includes('src="/main.tsx"')) {
    throw new Error("preview root does not serve the workspace fixture entry");
  }
