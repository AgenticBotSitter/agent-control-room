...
import { brokerConfiguration } from './worker-inbox-platform/lib/broker-read.mjs';
...
export function createInboxPoller({ read = readWorkerInbox, emit = console.log, workerId, repository, token, broker }) {
  let previous;
  const brokerState = {};
  return async () => {
    let result;
    try { result = { ok: true, actions: await read({ workerId, repository, token, includeReady: true, broker, brokerState }) }; }
    catch (error) {
      if (broker || error?.message === 'worker_inbox_rate_limited') return false;
      result = { ok: false, message: 'GitHub inbox unavailable. Existing assignments remain unresolved; retry on the next poll.' };
    }
...
async function main() {
  ...
  const poll = createInboxPoller({ workerId, repository, token: process.env.GITHUB_TOKEN, broker: brokerConfiguration() });
  let stopping = false;
  let timer;
  let wake;
