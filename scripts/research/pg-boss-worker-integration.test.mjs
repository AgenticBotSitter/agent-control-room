...
test('feed pickup and failure settlement run under the existing restricted operational worker role', async t => {
  ...
  await f.raw.exec(await readFile('db/roles/native_queue_worker_roles.sql', 'utf8'));
  await f.raw.exec(`CREATE ROLE feed_worker_test LOGIN INHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
    GRANT control_room_native_queue_worker TO feed_worker_test;
  ...
  const scoped = work => f.raw.transaction(async tx => {
