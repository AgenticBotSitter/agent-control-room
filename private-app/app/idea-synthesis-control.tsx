...
export function IdeaSynthesisControl({ sessionId, sessionDigest, runId, mode, refresh }: {
  sessionId: string; sessionDigest: string; runId?: string; mode: "legacy_panel" | "canonical_reviewed_tasks"; refresh?: () => void;
}) {
  ...
  async function prepare() {
    ...
    try { await client.synthesize(sessionId, mode === "legacy_panel" ? { sessionDigest, runId } : { sessionDigest, mode }); setSaved(true); refresh?.(); }
  ...
  return <div><p>Prepare an extractive recap and proposed experiment from reviewed saved results. This uses saved replies, not another AI judgment. No additional bot calls.</p>
