...
export function IdeaStartControl({ session, refresh }: { session: IdeaDetail["session"]; refresh?: () => void }) {
  ...
  async function start() {
    ...
    try { setReceipt(await client.propose(session.sessionId, { sessionDigest: session.sessionDigest, projectId, round: 1 })); }
    catch (reason) {
  ...
  return <div>
    ...
    {loadingProjects ? <p>Loading active projects…</p> : projects.length ? <label>Save tasks in project <select value={projectId ?? ""} disabled={busy || uncertain || !!receipt} onChange={event => setProjectId(event.target.value)}>
    ...
    <button type="button" disabled={loadingProjects || !projectId || busy || uncertain || !!receipt} onClick={() => void start()}>{busy ? "Preparing tasks…" : "Prepare first-round tasks"}</button>
    {error ? <p role="alert">{error}</p> : null}
    {receipt ? <p role="status">First-round tasks were saved for {receipt.receipts.length} participants. They are proposed only; review and assignment happen through the selected project.</p> : null}
