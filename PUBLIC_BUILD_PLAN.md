...
| --- | --- | --- |
| Core integration — advanced backend and integration lead | Wire real task admission → pg-boss → upstream identity → canonical result → review/revision → capacity release. Connect actual Codex and Hermes, not observations presented as execution. The Codex delivery/receipt/activation fence is available, but the next work is its local one-shot start authority/runtime and exact result schema/projection. Test replay, transaction failure, expiry, drain, restart and two projects. Includes A3–5, B1/B3/B6/B9 | Shared connector contract is settled; actual-interface gates in Q2/Q3 and authority qualifications in Q4 remain |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; exact-package schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable post-receipt activation fence is implemented: it binds one acknowledged delivery to one connection but grants no execution, retry, resume or thread-reading authority | Core integration: local one-shot start authority/runtime, exact result schema/projection, native restart read test, and canonical task/result wiring without auto-approval |
