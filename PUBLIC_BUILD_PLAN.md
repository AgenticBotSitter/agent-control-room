...
| DR-17 native execution/files | Selected Codex App Server `0.150.0-alpha.8` over owned stdio/TS transport and pinned hermes-gpt FastMCP continue/status/result | Managed queue, durable Codex post-receipt activation fence, pinned agent-message projection and an explicit-port private Codex initial/recover host are implemented with fake JSONL. MVP blocker is physical native acceptance and canonical publication, not another connector contest. Python Codex client remains fallback only. Finish native start/read qualification, lifecycle/results/recovery, truthful Hermes limitations and bounded artifacts |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; package-matching schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable post-receipt activation fence, one-shot start/read compositions, pinned agent-message projection and private explicit-port host are implemented. Fake JSONL carries one real bridge-recorded activation through exact durable start/recovery without a provider call. Stable run identity is separate from the effect claim and the owner permit binds the exact contained checkout path | Core integration: supply reviewed operator/native ports and credentials, qualify physical start/restart read, then enable canonical task/result/review/capacity wiring. Schema-qualified result text remains noncanonical until that native gate passes; old provisional pending packets are preserved and rejected/replanned, never rewritten or dual-accepted; a version upgrade requires a deliberate schema migration |
