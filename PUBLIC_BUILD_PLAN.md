...
| DR-15 rollback integrity | Selected existing etcd adapter as the independent authenticated compare-and-swap rollback detector; OpenBao is fallback only for a named permission/deployment failure | Qualification pending and required for persistent real completion writes: independent placement/credentials, both split-commit orders/lost acknowledgements, matching backup/WAL evidence and complete restore acceptance |
| DR-16 signing | Selected dedicated owner-controlled Ed25519 signer; ssh2 1.17 is only the bounded remote transport | Qualification pending and required for real effectful execution: dedicated custody/pins, exact consent, real peer permissions, timeout/cleanup, revocation, platform wiring and independent review. No ambient or forwarded personal key |
