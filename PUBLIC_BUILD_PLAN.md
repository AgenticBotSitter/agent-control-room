...
PR #109. That baseline retains the component implementation, public collaboration
...
| DR-17 native execution/files | Selected Codex App Server `0.150.0-alpha.8` over owned stdio/TS transport and pinned hermes-gpt FastMCP continue/status/result | Managed queue, durable Codex post-receipt activation fence and pinned agent-message result projection are implemented. MVP blocker is integration and real-machine acceptance, not another connector contest. Python Codex client remains fallback only. Finish native start/read qualification, lifecycle/results/recovery, truthful Hermes limitations and bounded artifacts |
...
| --- | --- | --- |
| Q1 | Settled: current public `main`, reconciled through PR #109, is the baseline | New issues record an immutable main-derived base when the maintainer-wide pause is lifted; preserved contributor branches keep ownership until accepted or explicitly handed off |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; package-matching schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable post-receipt activation fence, unwired one-shot start runtime and pinned agent-message result projection are implemented. The runtime binds the approved prompt and workspace to internally constructed read-only/user-reviewed requests, requires the response's exact selected `cliVersion`, persists a no-replay reservation and closes late I/O before reporting cleanup | Core integration: wire the reviewed local ports, qualify native start/restart read, then enable canonical task/result wiring. Schema-qualified result text remains noncanonical until that native gate passes; a version upgrade requires a deliberate schema migration |
