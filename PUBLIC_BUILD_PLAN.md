...
| DR-17 native execution/files | Selected Codex App Server `0.150.0-alpha.8` over owned stdio/TS transport and pinned hermes-gpt FastMCP continue/status/result | MVP blocker is integration and real-machine acceptance, not another connector contest. Python Codex client remains fallback only. Finish managed queue, local admission/start journal, lifecycle/results/recovery, truthful Hermes limitations and bounded artifacts |
