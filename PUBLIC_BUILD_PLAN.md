...
PR #110. That baseline retains the component implementation, public collaboration
...
| DR-12 sessions | Selected optional [Herdr](https://github.com/herdrdev/herdr) read-only observation adapter at `b99002ac99b09e00b4ca692436cb15a6b0d676f1`; operator-installed, not redistributed | Not an MVP blocker. If enabled, qualify exact executable/endpoint identity, owner-only socket/directory, selected-host isolation, cleanup, restart/disconnect and physical browser collection. No further terminal-system comparison |
...
| DR-14 monitoring | Selected built-in readiness and Needs attention for MVP; [Uptime Kuma](https://github.com/louislam/uptime-kuma) and [Beszel](https://github.com/henrygd/beszel) deferred as removable options | External monitoring is not an MVP blocker. Reopen only for a demonstrated alert or host-metric need; then prove protected health, restart/resource cost and actual alerts |
| DR-15 rollback integrity and recovery source | Selected existing etcd adapter as the independent authenticated compare-and-swap rollback detector; OpenBao is fallback only for a named permission/deployment failure. Selected operator-installed [restic](https://github.com/restic/restic) 0.19.1 (BSD-2-Clause) to retain the verified PostgreSQL dump plus artifact set outside the primary, with no backup credential in the application runtime | The effect-free fault matrix and versioned authenticated snapshot binding are implemented in source. Physical qualification still requires separate placement/credentials, both split-commit orders/lost acknowledgements, a real #63 PostgreSQL dump, matching #65 restored artifact readback and exact snapshot/checkpoint comparison. S3-compatible storage is not called append-only without provider proof |
| DR-16 signing | Selected dedicated owner-controlled Ed25519 signer; ssh2 1.17 is only the bounded remote transport | Qualification is required before critical or consequential external effects: dedicated custody/pins, exact consent, real peer permissions, timeout/cleanup, revocation, platform wiring and independent review. The narrow MVP permits only owner-submitted `allowExternalEffects=false` harness runs and never treats login/result text as signed approval. No ambient or forwarded personal key |
| DR-17 native execution/files | Selected Codex App Server `0.150.0-alpha.8` over owned stdio/TS transport and pinned hermes-gpt FastMCP continue/status/result | Managed queue, durable Codex post-receipt activation fence, pinned agent-message projection, explicit-port private initial/recover host and strict owned fake process-byte/JSONL adapter are implemented. The Codex-only publisher can persist exact bytes and one pending review target only after verifying an injected signed physical-qualification receipt plus the saved plan, activation and current admission; synthetic tests make no physical or completion claim. The first release qualifies Codex only through the reviewed Linux held-descriptor path. Local macOS Codex execution stays fail-closed because the evaluated mechanisms cannot bind pathname-based `CODEX_HOME` to the opened directory; #191 records the two proofs required to reopen it. MVP blockers are Linux physical native acceptance, live server composition, review completion and capacity release—not another connector contest or a Darwin port. The [Codex SDK fit evaluation](TOOLING_ACQUISITIONS.md#codex-typescript-sdk-fit-evaluation--2026-09-13) defers it as an optional batch adapter until it can meet passive recovery, approval and retirement requirements. Finish Linux start/read qualification, lifecycle/results/recovery, truthful Hermes limitations and bounded artifacts |

Selected-but-unqualified signing/checkpoint work is not silently optional security work. The lead
...
The first public MVP is explicitly narrowed to project/task orchestration and
owner-submitted low-risk Hermes/Codex runs with `allowExternalEffects=false`. Provider
inference and reversible writes inside an isolated task workspace are permitted;
deployment, publishing, messaging, purchasing, destructive cleanup, credential
changes and production/native administration are refused. The corresponding project,
task, attempt, result, review and artifact records may persist, but are not critical
approval evidence. Critical or consequential external effects remain disabled until
DR-15 and DR-16 pass physical qualification.
...
| --- | --- | --- |
| Core integration — advanced backend and integration lead | Wire real task admission → pg-boss → upstream identity → canonical result → review/revision → capacity release. Connect actual Codex and Hermes, not observations presented as execution. Codex delivery/receipt/activation, durable start/read host, strict owned process-byte adapter, pinned agent-message projection and reviewed Linux acquisition are available; next qualify Linux native start/restart read. Local macOS Codex execution is deferred under #191 and is not a first-release dependency. Test replay, transaction failure, expiry, drain, restart and two projects. Includes A3–5, B1/B3/B6/B9 | Shared connector contract is settled; actual-interface gates in Q2/Q3 and authority qualifications in Q4 remain |
| Product workspace — frontend/accessibility, any OS (#10/#1) | Lead-integrated pages now cover project lifecycle, task/result/revision, attention, explicit section routes, two configurations, responsive/keyboard/deep-link behavior and honest unavailable states. Remaining work is protected file delivery, live worker/capacity data, human assistive-technology/zoom/contrast acceptance and the real-harness journey. Includes A5–7, B8 | Protected files depend on #65; live worker data and final browser proof depend on #66/#68/#61. Do not rebuild the accepted product shell |
| Harness and host interoperability — Mac/Linux/Windows integration (#8/#2) | Real supported upstream mapping, version/capability inventory, independently installed worker setup, reconnect and bounded artifact transfer. Preserve unsupported status. Qualify Codex execution on Linux and Hermes on each claimed host; macOS remains a contributor/browser target while its local Codex path is refused. Windows checkout/install/test portability must leave Linux-only server custody intact. Includes B3–9 | Follow the shared connector contract and support matrix; actual-source API fit can proceed without credentials, live/native gates remain separately authorized |
...
| Worker operations (#68) | Install, enroll, version-check, reconnect, revoke and remove Hermes nodes on qualified Mac/Linux hosts and Codex nodes on Linux | Reuse accepted #8/#26 connectors; native proof is assembled only in #61. Local macOS Codex execution remains deferred under #191 |
...
| --- | --- | --- |
| Q1 | Settled: current public `main` is the integration baseline | Every ready or accepted implementation assignment records its own immutable main-derived base; accepted active branches retain ownership until integrated or explicitly handed off |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; package-matching schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable activation fence, start/read compositions, pinned agent-message projection, private explicit-port host, strict owned fake process-byte adapter and reviewed Linux held-descriptor acquisition are implemented. The adapter binds one fresh acquisition, rejects unsafe UTF-8/newline/buffer/exit/write conditions and withholds output on cleanup uncertainty. Fake JSONL carries one bridge-recorded activation through exact durable start/recovery without a provider call. Stable run identity is separate from the effect claim and the permit binds the exact contained checkout path. A qualification-gated Codex-only publisher now fake-tests exact artifact/result persistence and one pending review target while retaining a neutral discovered run anchor and false completion/capacity flags. Local macOS Codex execution remains fail-closed and is deferred under #191 rather than weakening `CODEX_HOME` custody | Core integration: qualify physical Linux start/restart read, compose the live server path, then finish review completion and capacity release. The persistence test uses an ephemeral synthetic signer and is not physical qualification; schema-qualified text remains noncanonical until the signed physical gate passes. Older provisional packets are preserved and rejected/replanned, never rewritten or dual-accepted. A Mac path reopens only after the two #191 proofs; it is not a first-release gate. The evaluated SDK remains a deferred optional batch adapter; a version upgrade requires deliberate schema migration |
...
| Q5 | Implemented: versioned non-secret product configuration for branding, modules, limits and templates is captured by startup and served only after gateway plus local owner authorization. One built artifact has now been verified with two isolated disposable configurations | Release: prove two installed configurations from the same candidate and verify no credentials in export; disposable proof is not deployment acceptance |
...
| Q7 | Settled initial mode: packaged Cloudflare assertion profile plus a fixed operator-configured RS256 gateway profile; externally enforced MFA only, generic OIDC not claimed | Security/operations: qualify the configured gateway and key source before claiming the second profile deployable; live direct-origin/MFA-policy verification remains deployment work |
...
| Q12 | Settled and integrated for the available services: current Control Room panels remain the base; WebUI-style anti-stuck semantics are covered by lost-request/lost-reply tests; the Desktop session strip stays deferred | Finish only dependency-backed UI: protected file delivery and live worker/capacity facts, then human accessibility and real-harness browser acceptance |
...
| Q14 | Settled and integrated for MVP: built-in health and Needs attention are first; owner attention is categorized, ranked and phrased as an actionable question. Kuma/Beszel stay optional external services | Operator visibility after MVP: add a service only with demonstrated notification/host-metric benefit and release evidence |
