...
| --- | --- | --- |
| Core integration — advanced backend and integration lead | Wire real task admission → pg-boss → upstream identity → canonical result → review/revision → capacity release. Connect actual Codex and Hermes, not observations presented as execution. Codex delivery/receipt/activation, durable start/read host, strict owned process-byte adapter and pinned agent-message projection are available; next supply the reviewed spawn/environment/credential implementation and qualify native start/restart read. Test replay, transaction failure, expiry, drain, restart and two projects. Includes A3–5, B1/B3/B6/B9 | Shared connector contract is settled; actual-interface gates in Q2/Q3 and authority qualifications in Q4 remain |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; package-matching schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable activation fence, start/read compositions, pinned agent-message projection, private explicit-port host and strict owned fake process-byte adapter are implemented. The adapter binds one fresh acquisition, rejects unsafe UTF-8/newline/buffer/exit/write conditions and withholds output on cleanup uncertainty. Fake JSONL carries one bridge-recorded activation through exact durable start/recovery without a provider call. Stable run identity is separate from the effect claim and the permit binds the exact contained checkout path. A qualification-gated Codex-only publisher now fake-tests exact artifact/result persistence and one pending review target while retaining a neutral discovered run anchor and false completion/capacity flags | Core integration: supply the reviewed actual spawn/executable/environment/credential port, qualify physical start/restart read, compose the live server path, then finish review completion and capacity release. The persistence test uses an ephemeral synthetic signer and is not physical qualification; schema-qualified text remains noncanonical until the signed physical gate passes. Older provisional packets are preserved and rejected/replanned, never rewritten or dual-accepted. The evaluated SDK remains a deferred optional batch adapter; a version upgrade requires deliberate schema migration |
