...
| DR-17 native execution/files | Selected Codex App Server `0.150.0-alpha.8` over owned stdio/TS transport and pinned hermes-gpt FastMCP continue/status/result | Managed queue and durable Codex post-receipt activation fence are implemented. MVP blocker is integration and real-machine acceptance, not another connector contest. Python Codex client remains fallback only. Finish local one-shot start authority/runtime, exact result schema/projection, lifecycle/results/recovery, truthful Hermes limitations and bounded artifacts |
...
| --- | --- | --- |
| Core integration — advanced backend and integration lead | Wire real task admission → pg-boss → upstream identity → canonical result → review/revision → capacity release. Connect actual Codex and Hermes, not observations presented as execution. Codex delivery/receipt/activation and an unwired, fake-tested one-shot local start runtime are available; next wire its durable/workspace/owned-process ports and finish exact result schema/projection. Test replay, transaction failure, expiry, drain, restart and two projects. Includes A3–5, B1/B3/B6/B9 | Shared connector contract is settled; actual-interface gates in Q2/Q3 and authority qualifications in Q4 remain |
...
| Q3 | Settled contract: Codex uses App Server `0.150.0-alpha.8` over parent-owned stdio; exact-package schema evidence is bound to read-only `thread/read(includeTurns: true)`. The durable post-receipt activation fence and unwired one-shot start runtime are implemented. The runtime binds the approved prompt and workspace to internally constructed read-only/user-reviewed requests, requires the response's exact selected `cliVersion`, persists a no-replay reservation and closes late I/O before reporting cleanup | Core integration: wire the reviewed local ports, exact result schema/projection, native start/restart read test, and canonical task/result wiring. Fake transport tests do not claim a provider call or physical process qualification; a version upgrade requires a deliberate schema migration |
