...
CREATE TABLE control_news_story_versions (
...
CREATE INDEX control_news_story_latest ON control_news_story_versions
  (tenant_id, workspace_id, project_id, story_id, sequence DESC);
CREATE TRIGGER control_news_story_immutable BEFORE UPDATE OR DELETE ON control_news_story_versions
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();

CREATE TABLE control_news_research_proposals (
  tenant_id text NOT NULL,
  workspace_id text NOT NULL,
  project_id text NOT NULL,
...
CREATE TRIGGER control_news_proposal_immutable BEFORE UPDATE OR DELETE ON control_news_research_proposals
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();
