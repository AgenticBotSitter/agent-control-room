...
CREATE TABLE control_news_source_observations (
...
CREATE INDEX control_news_source_latest ON control_news_source_observations
  (tenant_id,workspace_id,project_id,source_id,checked_at DESC,sequence DESC);
CREATE TRIGGER control_news_source_immutable BEFORE UPDATE OR DELETE ON control_news_source_observations
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();
CREATE TRIGGER control_news_source_no_truncate BEFORE TRUNCATE ON control_news_source_observations
