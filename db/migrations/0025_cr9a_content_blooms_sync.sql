-- CR-9A: isolated, source-scheduled Content Blooms read synchronization.
...
CREATE TABLE control_content_blooms_releases (
...
CREATE TABLE control_content_blooms_state (
  tenant_id text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
...
CREATE TABLE control_content_blooms_read_receipts (
  tenant_id text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
...
CREATE TABLE control_content_blooms_streams (
  tenant_id text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
  workspace_id text NOT NULL,
  project_id text NOT NULL,
  adapter_id text NOT NULL,
  ...
  FOREIGN KEY (tenant_id,receipt_digest) REFERENCES control_content_blooms_read_receipts(tenant_id,receipt_digest) ON DELETE RESTRICT
);

CREATE TABLE control_content_blooms_record_history (
  tenant_id text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
  workspace_id text NOT NULL,
  project_id text NOT NULL,
  adapter_id text NOT NULL,
...
CREATE TABLE control_content_blooms_current_records (
  tenant_id text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
  workspace_id text NOT NULL,
  project_id text NOT NULL,
  adapter_id text NOT NULL,
...
CREATE INDEX idx_content_blooms_current_scope
  ON control_content_blooms_current_records(tenant_id,workspace_id,project_id,kind,source_record_id);

CREATE TRIGGER control_content_blooms_releases_append_only
  BEFORE UPDATE OR DELETE ON control_content_blooms_releases
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();
CREATE TRIGGER control_content_blooms_read_receipts_append_only
  BEFORE UPDATE OR DELETE ON control_content_blooms_read_receipts
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();
CREATE TRIGGER control_content_blooms_record_history_append_only
  BEFORE UPDATE OR DELETE ON control_content_blooms_record_history
  FOR EACH ROW EXECUTE FUNCTION reject_append_only_mutation();
